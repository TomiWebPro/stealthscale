...
let
  cfg = config.services.stealthscale;

  dataDir = "/var/lib/stealthscale";
  runDir = "/run/stealthscale";
  ...
  cliConfig = {
    ...
    unix_socket = "${runDir}/stealthscale.sock";
  };

  settingsFormat = pkgs.formats.yaml { };
  cliConfigFile = settingsFormat.generate "stealthscale.yaml" cliConfig;

  assertRemovedOption = option: message: {
    assertion = !lib.hasAttrByPath option cfg;
    message =
      "The option `services.stealthscale.${lib.options.showOption option}` was removed. " + message;
  };
in
{
  # Disable the upstream NixOS module to prevent conflicts
  disabledModules = [ "services/networking/stealthscale.nix" ];
  ...
  options = {
    services.stealthscale = {
      ...
      configFile = lib.mkOption {
        type = lib.types.path;
        readOnly = true;
        default = settingsFormat.generate "stealthscale.yaml" cfg.settings;
        defaultText = lib.literalExpression ''(pkgs.formats.yaml { }).generate "stealthscale.yaml" config.services.stealthscale.settings'';
        description = ''
          Path to the configuration file of stealthscale.
        '';
      };
      ...
      user = lib.mkOption {
        ...
        description = ''
          ...
          ensuring the user exists before the stealthscale service starts.
          :::
        '';
      };
      ...
      address = lib.mkOption {
        ...
        default = "127.0.0.1";
        description = ''
          Listening address of stealthscale.
        '';
        example = "0.0.0.0";
      };
      ...
      port = lib.mkOption {
        ...
        default = 8080;
        description = ''
          Listening port of stealthscale.
        '';
        example = 443;
      };
            ...
            database = {
              ...
              postgres = {
                ...
                password_file = lib.mkOption {
                  ...
                  default = null;
                  example = "/run/keys/stealthscale-dbpassword";
                  description = ''
  ...
  imports = with lib; [
    (mkRenamedOptionModule
      [ "services" "stealthscale" "derp" "autoUpdate" ]
      [ "services" "stealthscale" "settings" "derp" "auto_update_enabled" ]
    )
    (mkRenamedOptionModule
      [ "services" "stealthscale" "derp" "auto_update_enable" ]
      [ "services" "stealthscale" "settings" "derp" "auto_update_enabled" ]
    )
    (mkRenamedOptionModule
      [ "services" "stealthscale" "derp" "paths" ]
      [ "services" "stealthscale" "settings" "derp" "paths" ]
    )
    (mkRenamedOptionModule
      [ "services" "stealthscale" "derp" "updateFrequency" ]
      [ "services" "stealthscale" "settings" "derp" "update_frequency" ]
    )
    (mkRenamedOptionModule
      [ "services" "stealthscale" "derp" "urls" ]
      [ "services" "stealthscale" "settings" "derp" "urls" ]
    )
    (mkRenamedOptionModule
      [ "services" "stealthscale" "ephemeralNodeInactivityTimeout" ]
      [ "services" "stealthscale" "settings" "ephemeral_node_inactivity_timeout" ]
    )
    (mkRenamedOptionModule
      [ "services" "stealthscale" "logLevel" ]
      [ "services" "stealthscale" "settings" "log" "level" ]
    )
    (mkRenamedOptionModule
      [ "services" "stealthscale" "openIdConnect" "clientId" ]
      [ "services" "stealthscale" "settings" "oidc" "client_id" ]
    )
    (mkRenamedOptionModule
      [ "services" "stealthscale" "openIdConnect" "clientSecretFile" ]
      [ "services" "stealthscale" "settings" "oidc" "client_secret_path" ]
    )
    (mkRenamedOptionModule
      [ "services" "stealthscale" "openIdConnect" "issuer" ]
      [ "services" "stealthscale" "settings" "oidc" "issuer" ]
    )
    (mkRenamedOptionModule
      [ "services" "stealthscale" "serverUrl" ]
      [ "services" "stealthscale" "settings" "server_url" ]
    )
    (mkRenamedOptionModule
      [ "services" "stealthscale" "tls" "certFile" ]
      [ "services" "stealthscale" "settings" "tls_cert_path" ]
    )
    (mkRenamedOptionModule
      [ "services" "stealthscale" "tls" "keyFile" ]
      [ "services" "stealthscale" "settings" "tls_key_path" ]
    )
    (mkRenamedOptionModule
      [ "services" "stealthscale" "tls" "letsencrypt" "challengeType" ]
      [ "services" "stealthscale" "settings" "tls_letsencrypt_challenge_type" ]
    )
    (mkRenamedOptionModule
      [ "services" "stealthscale" "tls" "letsencrypt" "hostname" ]
      [ "services" "stealthscale" "settings" "tls_letsencrypt_hostname" ]
    )
    (mkRenamedOptionModule
      [ "services" "stealthscale" "tls" "letsencrypt" "httpListen" ]
      [ "services" "stealthscale" "settings" "tls_letsencrypt_listen" ]
    )
  ...
  config = lib.mkIf cfg.enable {
    ...
    services.stealthscale.settings = lib.mkMerge [
      cliConfig
      {
    ...
    environment = {
      # StealthScale CLI needs a minimal config to be able to locate the unix socket
      # to talk to the server instance.
      etc."stealthscale/config.yaml".source = cliConfigFile;
    ...
    systemd.services.stealthscale = {
      ...
      script = ''
        ${lib.optionalString (cfg.settings.database.postgres.password_file != null) ''
          export STEALTHSCALE_DATABASE_POSTGRES_PASS="$(head -n1 ${lib.escapeShellArg cfg.settings.database.postgres.password_file})"
        ''}
      ...
      serviceConfig =
        ...
        in
        {
          ...
          # Hardening options
          RuntimeDirectory = "stealthscale";
          # Allow stealthscale group access so users can be added and use the CLI.
