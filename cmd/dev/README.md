...
Starts a stealthscale server on localhost with a pre-created user and
pre-auth key. Pair with `mts` to add real tailscale nodes.

## Quick start

```bash
# Terminal 1: start stealthscale
go run ./cmd/dev
...
# Connect to stealthscale (use the auth key printed by cmd/dev)
...
| -------- | ------- | ---------------------------- |
| `--port` | 8080    | stealthscale listen port        |
...
1. Builds the stealthscale binary into a temp directory
2. Writes a minimal dev config (SQLite, public DERP, debug logging)
3. Starts `stealthscale serve` as a subprocess
...
6. Blocks until Ctrl+C, then kills stealthscale

State lives in `/tmp/stealthscale-dev-*/`. Pass `--keep` to preserve it
...
## Managing stealthscale

The banner prints the full path to the built binary and config. Use it
for any stealthscale CLI command:

```bash
/tmp/stealthscale-dev-*/stealthscale -c /tmp/stealthscale-dev-*/config.yaml nodes list
/tmp/stealthscale-dev-*/stealthscale -c /tmp/stealthscale-dev-*/config.yaml users list
```
