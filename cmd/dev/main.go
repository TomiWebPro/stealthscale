// cmd/dev starts a local stealthscale development server with a pre-created
...
var (
	port = flag.Int("port", 8080, "stealthscale listen port")
...
unix_socket: %s/stealthscale.sock
...
func run() error {
	...
	tmpDir, err := os.MkdirTemp("", "stealthscale-dev-")
	if err != nil {
	...
	// Build stealthscale.
	fmt.Println("Building stealthscale...")

	hsBin := filepath.Join(tmpDir, "stealthscale")

	ctx, stop := signal.NotifyContext(context.Background(), syscall.SIGINT, syscall.SIGTERM)
	defer stop()

	build := exec.CommandContext(ctx, "go", "build", "-o", hsBin, "./cmd/stealthscale")
	build.Stdout = os.Stdout
	build.Stderr = os.Stderr

	err = build.Run()
	if err != nil {
		return fmt.Errorf("building stealthscale: %w", err)
	}

	// Start stealthscale serve.
	fmt.Println("Starting stealthscale server...")
	...
	err = serve.Start()
	if err != nil {
		return fmt.Errorf("starting stealthscale: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("waiting for stealthscale: %w", err)
	}
...
`,
	...
	if err != nil {
		...
		return fmt.Errorf("stealthscale exited: %w", err)
	}
...
// runHS executes a stealthscale CLI command and returns its stdout.
