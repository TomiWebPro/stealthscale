...
func bypassDatabase() (*db.HSDatabase, error) {
	...
	d, err := db.NewStealthScaleDatabase(cfg)
	if err != nil {
...
func openBypassDB(cmd *cobra.Command) (*db.HSDatabase, error) {
	if !confirmAction(cmd, "DO NOT run this command if an instance of stealthscale is running, are you sure stealthscale is not running?") {
...
func init() {
	...
	getPolicy.Flags().BoolP(bypassFlag, "", false, "Uses the stealthscale config to directly access the database, bypassing the API and does not require the server to be running")
	policyCmd.AddCommand(getPolicy)

	setPolicy.Flags().StringP("file", "f", "", "Path to a policy file in HuJSON format")
	setPolicy.Flags().BoolP(bypassFlag, "", false, "Uses the stealthscale config to directly access the database, bypassing the API and does not require the server to be running")
...
var checkPolicy = &cobra.Command{
	...
	running stealthscale over its API; pass --` + bypassFlag + ` to
	open the database directly when stealthscale is not running.`,
	RunE: func(cmd *cobra.Command, args []string) error {
		policyPath, _ := cmd.Flags().GetString("file")
