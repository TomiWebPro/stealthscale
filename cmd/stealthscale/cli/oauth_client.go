...
// oauthTailnet is the single StealthScale tailnet the v2 API addresses as "-".
const oauthTailnet = "-"
...
the Kubernetes operator, tscli, ...), so those can drive StealthScale unchanged.
		...
		return printListOutput(cmd, clients, func() error {
			...
			for _, c := range clients {
				rows = append(rows, []string{
					...
					c.Created.Format(StealthScaleDateTimeFormat),
				})
			}
