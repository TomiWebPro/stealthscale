...
func init() {
	...
	nodeCmd.AddCommand(nodeVLESSCommand)
...
var nodeCmd = &cobra.Command{
	Use:     "nodes",
	Short:   "Manage the nodes of StealthScale",
	Aliases: []string{"node"},
}
...
var registerNodeCmd = &cobra.Command{
	...
	Deprecated: "use 'stealthscale auth register --auth-id <id> --user <user>' instead",
...
var nodeVLESSCommand = &cobra.Command{
	Use:   "vless",
	Short: "Show the VLESS endpoint configuration for a node",
	Args:  cobra.ExactArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		nodeID, err := types.ParseNodeID(args[0])
		if err != nil {
...
based on the current configuration of StealthScale.
...
func nodesToPtables(nodes []clientv1.Node) (pterm.TableData, error) {
	...
	for _, node := range nodes {
		...
		if node.LastSeen != nil {
			lastSeenTime = node.LastSeen.Format(StealthScaleDateTimeFormat)
		}

		expiryTime := "N/A"
		if node.Expiry != nil {
			expiryTime = node.Expiry.Format(StealthScaleDateTimeFormat)
		}
