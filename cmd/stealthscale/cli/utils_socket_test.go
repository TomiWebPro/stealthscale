...
// TestDialStealthScaleSocketRetriesUntilPresent proves the CLI socket dialer
...
func TestDialStealthScaleSocketRetriesUntilPresent(t *testing.T) {
	sock := filepath.Join(t.TempDir(), "stealthscale.sock")
	...
	go func() {
		conn, err := dialStealthScaleSocket(ctx, sock)
...
// TestDialStealthScaleSocketRespectsDeadline proves the retry is bounded by the
// context: when the socket never appears, the dialer returns an error around the
// deadline instead of hanging.
func TestDialStealthScaleSocketRespectsDeadline(t *testing.T) {
