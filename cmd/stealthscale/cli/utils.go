...
const (
	StealthScaleDateTimeFormat = "2006-01-02 15:04:05"
...
var (
	errAPIKeyNotSet     = errors.New("STEALTHSCALE_CLI_API_KEY environment variable needs to be set")
...
func newStealthScaleServerWithConfig() (*hscontrol.StealthScale, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("creating new stealthscale: %w", err)
	}
...
func clientRunE(
	...
	return func(cmd *cobra.Command, args []string) error {
		ctx, client, cancel, err := newStealthScaleCLIWithConfig()
		if err != nil {
			return fmt.Errorf("connecting to stealthscale: %w", err)
		}
		defer cancel()
...
// newStealthScaleCLIWithConfig builds an HTTP client for the StealthScale v1 API.
//
...
func newStealthScaleCLIWithConfig() (context.Context, *clientv1.ClientWithResponses, context.CancelFunc, error) {
	...
	if address == "" {
		log.Debug().
			...
			Msgf("STEALTHSCALE_CLI_ADDRESS environment is not set, connecting to unix socket.")
	...
	httpClient := &http.Client{
		Transport: &http.Transport{
			DialContext: func(ctx context.Context, _, _ string) (net.Conn, error) {
				return dialStealthScaleSocket(ctx, socketPath)
			},
		},
	}
...
// newRemoteClient builds an API client for a remote StealthScale over HTTPS,
