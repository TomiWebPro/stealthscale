...
func checkGitRepository(ctx context.Context) DoctorResult {
	...
	if err != nil {
		return fail(
			...
			"Run from within the stealthscale git repository",
			"Clone the repository: git clone https://github.com/tomiwebpro/stealthscale.git",
		)
	}
...
func checkRequiredFiles(_ context.Context) DoctorResult {
	...
	if len(missingFiles) > 0 {
		return fail(
			...
			"Ensure you're in the stealthscale project root directory",
			"Check that integration/ directory exists",
			"Verify this is a complete stealthscale repository",
		)
	}
