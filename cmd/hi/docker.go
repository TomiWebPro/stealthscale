...
func runTestContainer(ctx context.Context, config *RunConfig) error {
	...
	containerName := "stealthscale-test-suite-" + runID
...
func createGoTestContainer(ctx context.Context, cli *client.Client, config *RunConfig, containerName, logsDir string, goTestCmd []string) (container.CreateResponse, error) {
	...
	env := []string{
		fmt.Sprintf("STEALTHSCALE_INTEGRATION_POSTGRES=%d", boolToInt(config.UsePostgres)),
		"STEALTHSCALE_INTEGRATION_RUN_ID=" + runID,
	}
	...
	// Pass through all STEALTHSCALE_INTEGRATION_* environment variables
	for _, e := range os.Environ() {
		if strings.HasPrefix(e, "STEALTHSCALE_INTEGRATION_") {
			// Skip the ones we already set explicitly
			if strings.HasPrefix(e, "STEALTHSCALE_INTEGRATION_POSTGRES=") ||
				strings.HasPrefix(e, "STEALTHSCALE_INTEGRATION_RUN_ID=") {
				continue
			}
	...
	goCache := os.Getenv("STEALTHSCALE_INTEGRATION_GO_CACHE")
	goBuildCache := os.Getenv("STEALTHSCALE_INTEGRATION_GO_BUILD_CACHE")
...
// listControlFiles displays the stealthscale test artifacts created in the control logs directory.
func listControlFiles(logsDir string) {
	...
	for _, entry := range entries {
		name := entry.Name()
		// Only show stealthscale (hs-*) files and directories
...
func extractContainerLogs(ctx context.Context, cli *client.Client, containerID, containerName, logsDir string, verbose bool) error {
	...
	// Create log files following the stealthscale naming convention
