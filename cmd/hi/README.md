# hi — StealthScale Integration test runner
...
| `--hs-memory-limit` | `0`            | Fail if any stealthscale container exceeds N MB (0 = disabled)                 |
...
├── hs-<test>-<hash>.stderr.log        # stealthscale server errors
├── hs-<test>-<hash>.stdout.log        # stealthscale server output
...
1. **`hs-*.stderr.log`** — stealthscale server errors, panics, policy
