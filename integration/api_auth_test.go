...
// - https://github.com/tomiwebpro/stealthscale/issues/2809
// - https://github.com/tomiwebpro/stealthscale/pull/2810
//
...
func TestAPIAuthenticationBypass(t *testing.T) {
	...
	err = scenario.CreateStealthScaleEnv([]tsic.Option{}, hsic.WithTestName("apiauthbypass"))
	require.NoError(t, err)
	...
	assert.EventuallyWithT(t, func(ct *assert.CollectT) {
		apiKeyOutput, err := stealthscale.Execute(
			[]string{
				"stealthscale",
				"apikeys",
				"create",
	...
	endpoint := stealthscale.GetEndpoint()
...
func TestAPIAuthenticationBypassCurl(t *testing.T) {
	...
	err = scenario.CreateStealthScaleEnv([]tsic.Option{}, hsic.WithTestName("apiauthcurl"))
	require.NoError(t, err)
	...
	t.Run("Curl_NoAuth", func(t *testing.T) {
		// Execute curl from inside the stealthscale container without auth
		curlOutput, err := stealthscale.Execute(
			[]string{
				"curl",
				"-s",
				"-w",
...
func TestRemoteCLIAuthenticationBypass(t *testing.T) {
	...
	remoteAddr := strings.TrimPrefix(strings.TrimPrefix(stealthscale.GetEndpoint(), "https://"), "http://")

	t.Run("Remote_NoAPIKey", func(t *testing.T) {
		// Test 1: Try to use CLI without API key (should fail)
		// When STEALTHSCALE_CLI_ADDRESS is set but STEALTHSCALE_CLI_API_KEY is not set,
		// the CLI should fail immediately
		_, err := stealthscale.Execute(
			[]string{
				"sh", "-c",
				fmt.Sprintf("STEALTHSCALE_CLI_ADDRESS=%s STEALTHSCALE_CLI_INSECURE=true stealthscale users list --output json 2>&1", remoteAddr),
			},
		)
	...
	t.Run("Remote_InvalidAPIKey", func(t *testing.T) {
		...
		output, err := stealthscale.Execute(
			[]string{
				"sh", "-c",
				fmt.Sprintf("STEALTHSCALE_CLI_ADDRESS=%s STEALTHSCALE_CLI_API_KEY=invalid-key-12345 STEALTHSCALE_CLI_INSECURE=true stealthscale users list --output json 2>&1", remoteAddr),
			},
		)
	...
	t.Run("Remote_ValidAPIKey", func(t *testing.T) {
		...
		output, err := stealthscale.Execute(
			[]string{
				"sh", "-c",
				fmt.Sprintf("STEALTHSCALE_CLI_ADDRESS=%s STEALTHSCALE_CLI_API_KEY=%s STEALTHSCALE_CLI_INSECURE=true stealthscale users list --output json", remoteAddr, validAPIKey),
			},
		)
...
// TestCLIWithConfigAuthenticationBypass tests that the stealthscale CLI
...
`, remoteAddr, validAPIKey)
	...
	t.Run("CLI_Config_NoAPIKey", func(t *testing.T) {
		...
		err := stealthscale.WriteFile("/tmp/config_no_key.yaml", []byte(configWithoutKey))
		require.NoError(t, err)
	...
	t.Run("CLI_Config_InvalidAPIKey", func(t *testing.T) {
		...
		err := stealthscale.WriteFile("/tmp/config_invalid_key.yaml", []byte(configWithInvalidKey))
		require.NoError(t, err)
		...
		output, err := stealthscale.Execute(
			[]string{
				"sh", "-c",
				"stealthscale --config /tmp/config_invalid_key.yaml users list --output json 2>&1",
			},
		)
	...
	t.Run("CLI_Config_ValidAPIKey", func(t *testing.T) {
		...
		err := stealthscale.WriteFile("/tmp/config_valid_key.yaml", []byte(configWithValidKey))
		require.NoError(t, err)
