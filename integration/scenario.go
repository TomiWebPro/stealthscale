...
var usePostgresForTest = envknob.Bool("STEALTHSCALE_INTEGRATION_POSTGRES")

var (
	errNoStealthScaleAvailable = errors.New("no stealthscale available")
...
type ScenarioSpec struct {
	...
	// OIDCSkipUserCreation, if true, skips creating users via stealthscale CLI
...
func (s *Scenario) AddNetworkWithSubnet(name, subnet string) (*dockertest.Network, error) {
	...
	testSuiteName := "stealthscale-test-suite"
	if runID := dockertestutil.GetIntegrationRunID(); runID != "" {
		testSuiteName = "stealthscale-test-suite-" + runID
	}
...
/// StealthScale related stuff
// Note: These functions assume that there is a _single_ stealthscale instance for now

// StealthScale returns a [ControlServer] instance based on hsic ([hsic.StealthScaleInContainer]).
...
// TODO(kradalby): make port and stealthscale configurable, multiple instances support?
func (s *Scenario) StealthScale(opts ...hsic.Option) (ControlServer, error) {
	s.mu.Lock()
	defer s.mu.Unlock()

	if stealthscale, ok := s.controlServers.Load("stealthscale"); ok {
		return stealthscale, nil
	}
	...
	stealthscale, err := hsic.New(s.pool, s.Networks(), opts...)
	if err != nil {
		return nil, fmt.Errorf("creating stealthscale container: %w", err)
	}

	err = stealthscale.WaitForRunning()
	if err != nil {
		return nil, fmt.Errorf("reaching stealthscale container: %w", err)
	}

	s.controlServers.Store("stealthscale", stealthscale)
...
// StealthScale instance on behalf of the [Scenario].
func (s *Scenario) CreatePreAuthKey(
	user uint64,
	reusable bool,
	ephemeral bool,
) (*clientv1.PreAuthKey, error) {
	if stealthscale, err := s.StealthScale(); err == nil { //nolint:noinlineerr
		key, err := stealthscale.CreateAuthKey(user, reusable, ephemeral)
		if err != nil {
	...
	return nil, fmt.Errorf("creating user: %w", errNoStealthScaleAvailable)
}

// CreatePreAuthKeyWithOptions creates a "pre authorised key" with the specified options
// to be created in the StealthScale instance on behalf of the [Scenario].
func (s *Scenario) CreatePreAuthKeyWithOptions(opts hsic.AuthKeyOptions) (*clientv1.PreAuthKey, error) {
	stealthscale, err := s.StealthScale()
	if err != nil {
		return nil, fmt.Errorf("creating preauth key with options: %w", errNoStealthScaleAvailable)
	}

	key, err := stealthscale.CreateAuthKeyWithOptions(opts)
	if err != nil {
...
func (s *Scenario) CreatePreAuthKeyWithTags(
	...
	stealthscale, err := s.StealthScale()
	if err != nil {
		return nil, fmt.Errorf("creating preauth key with tags: %w", errNoStealthScaleAvailable)
	}

	key, err := stealthscale.CreateAuthKeyWithTags(user, reusable, ephemeral, tags)
	if err != nil {
...
func (s *Scenario) CreateUser(user string) (*clientv1.User, error) {
	if stealthscale, err := s.StealthScale(); err == nil { //nolint:noinlineerr
		u, err := stealthscale.CreateUser(user)
		if err != nil {
			return nil, fmt.Errorf("creating user: %w", err)
		}
...
func (s *Scenario) CreateTailscaleNode(
	...
	cert := stealthscale.GetCert()
	hostname := stealthscale.GetHostname()
	...
	opts = append(
		...
		tsic.WithStealthScaleName(hostname),
	)
...
func (s *Scenario) CreateTailscaleNodesInUser(
	...
	if user, ok := s.users[userStr]; ok {
		...
		for i := range count {
			...
			// Get stealthscale IP in this network for /etc/hosts fallback DNS
			stealthscaleIP := stealthscale.GetIPInNetwork(network)
			extraHosts := []string{hostname + ":" + stealthscaleIP}
...
func (s *Scenario) CreateStealthScaleEnvWithLoginURL(
	tsOpts []tsic.Option,
	opts ...hsic.Option,
) error {
	return s.createStealthScaleEnv(true, tsOpts, opts...)
}

func (s *Scenario) CreateStealthScaleEnv(
	tsOpts []tsic.Option,
	opts ...hsic.Option,
) error {
	return s.createStealthScaleEnv(false, tsOpts, opts...)
}

// CreateStealthScaleEnv starts the stealthscale environment and the clients
...
func (s *Scenario) createStealthScaleEnv(
	...
	return s.createStealthScaleEnvWithTags(withURL, tsOpts, nil, "", opts...)
}

// createStealthScaleEnvWithTags starts the stealthscale environment and the clients
...
func (s *Scenario) createStealthScaleEnvWithTags(
	withURL bool,
	tsOpts []tsic.Option,
	...
	stealthscale, err := s.StealthScale(opts...)
	if err != nil {
		return err
	}
	...
	for _, user := range s.spec.Users {
		...
		if s.spec.OIDCSkipUserCreation {
			// Only register locally — OIDC login will create the stealthscale user.
			s.mu.Lock()
		...
		if withURL {
			err = s.RunTailscaleUpWithURL(user, stealthscale.GetEndpoint())
			if err != nil {
				return err
			}
		} else {
			...
			err = s.RunTailscaleUp(user, stealthscale.GetEndpoint(), key.Key)
			if err != nil {
				return err
			}
		}
		...
		for _, client := range user.Clients {
			...
			user.joinWaitGroup.Go(func() error {
				...
				if !strings.Contains(loginURL.String(), "/oidc/") {
					_ = s.runStealthScaleRegister(userStr, body)
				}
...
func submitConfirmForm(
	...
	before, _, ok := strings.Cut(htmlBody, `name="stealthscale_register_confirm"`)
	if !ok {
	...
	formData := url.Values{
		"stealthscale_register_confirm": {csrfToken},
	}
...
func (s *Scenario) runStealthScaleRegister(userStr string, body string) error {
	...
	if stealthscale, err := s.StealthScale(); err == nil { //nolint:noinlineerr
		_, err = stealthscale.Execute(
			[]string{"stealthscale", "auth", "register", "--user", userStr, "--auth-id", key},
		)
		if err != nil {
	...
	return fmt.Errorf("finding stealthscale: %w", errNoStealthScaleAvailable)
}
...
func (s *Scenario) runMockOIDC(accessTTL time.Duration, users []mockoidc.MockUser) error {
	...
	mockOidcOptions := &dockertest.RunOptions{
		Name:         hostname,
		Cmd:          []string{"stealthscale", "mockoidc"},
	...
	// stealthscale needs to set up the provider with a specific
	...
	log.Println("Waiting for stealthscale mock oidc to be ready for tests")
	...
	if err := s.pool.Retry(func() error { //nolint:noinlineerr
		...
		if err != nil {
			log.Printf("stealthscale mock OIDC tests is not ready: %s\n", err)
	...
	log.Printf("stealthscale mock oidc is ready for tests at %s", hostEndpoint)
