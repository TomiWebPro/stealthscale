...
func TestAuthWebFlowLogoutAndReloginSameUser(t *testing.T) {
	...
	validateLogoutComplete(t, stealthscale, expectedNodes)

	t.Logf("all clients logged out")

	for _, userName := range spec.Users {
		err = scenario.RunTailscaleUpWithURL(userName, stealthscale.GetEndpoint())
		if err != nil {
			t.Fatalf("failed to run tailscale up (%q): %s", stealthscale.GetEndpoint(), err)
		}
	}
	...
	validateReloginComplete(t, stealthscale, expectedNodes)
...
//
// This test validates the "user switching" behavior in stealthscale's web authentication flow:
...
// or when consolidating multiple user accounts. It ensures that stealthscale properly handles
// the security implications of user switching while maintaining node persistence in the database.
//
// The test uses stealthscale's web authentication flow, which is the most user-friendly method
...
func TestAuthWebFlowLogoutAndReloginNewUser(t *testing.T) {
	...
	for _, client := range allClients {
		...
		// This simulates: stealthscale auth register --auth-id <id> --user user1
		_ = scenario.runStealthScaleRegister("user1", body)
	}
	...
	validateReloginComplete(t, stealthscale, expectedUser1Nodes)
