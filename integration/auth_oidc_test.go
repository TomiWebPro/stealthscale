...
func TestOIDCAuthenticationPingAll(t *testing.T) {
	...
	oidcMap := map[string]string{
		"STEALTHSCALE_OIDC_ISSUER":             scenario.mockOIDC.Issuer(),
		"STEALTHSCALE_OIDC_CLIENT_ID":          scenario.mockOIDC.ClientID(),
		"CREDENTIALS_DIRECTORY_TEST":        "/tmp",
		"STEALTHSCALE_OIDC_CLIENT_SECRET_PATH": "${CREDENTIALS_DIRECTORY_TEST}/hs_client_oidc_secret",
	}
	...
	err = scenario.CreateStealthScaleEnvWithLoginURL(
		nil,
	...
	listUsers, err := stealthscale.ListUsers()
	require.NoError(t, err)
...
func TestOIDCExpireNodesBasedOnTokenExpiry(t *testing.T) {
	...
	oidcMap := map[string]string{
		"STEALTHSCALE_OIDC_ISSUER":                scenario.mockOIDC.Issuer(),
		"STEALTHSCALE_OIDC_CLIENT_ID":             scenario.mockOIDC.ClientID(),
		"STEALTHSCALE_OIDC_CLIENT_SECRET":         scenario.mockOIDC.ClientSecret(),
		"STEALTHSCALE_OIDC_USE_EXPIRY_FROM_TOKEN": "1",
	}
...
func TestOIDC024UserCreation(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			emailVerified: false,
			cliUsers:      []string{"user1.stealthscale.net", "user2.stealthscale.net"},
			oidcUsers:     []string{"user1", "user2"},
			want: func(iss string) []*clientv1.User {
				return []*clientv1.User{
					{
						Id:    "1",
						Name:  "user1.stealthscale.net",
						Email: "user1.stealthscale.net@test.no",
					},
					{
						Id:         "2",
						Name:       "user1",
					...
					{
						Id:    "3",
						Name:  "user2.stealthscale.net",
						Email: "user2.stealthscale.net@test.no",
					},
					{
						Id:         "4",
						Name:       "user2",
...
func TestOIDCAuthenticationWithPKCE(t *testing.T) {
	...
	oidcMap := map[string]string{
		...
		"STEALTHSCALE_OIDC_CLIENT_SECRET_PATH": "${CREDENTIALS_DIRECTORY_TEST}/hs_client_oidc_secret",
		"CREDENTIALS_DIRECTORY_TEST":        "/tmp",
		"STEALTHSCALE_OIDC_PKCE_ENABLED":       "1", // Enable PKCE
	}
...
func TestOIDCReloginSameNodeNewUser(t *testing.T) {
	...
	u, err := ts.LoginWithURL(stealthscale.GetEndpoint())
	require.NoError(t, err)
	...
	assert.EventuallyWithT(t, func(ct *assert.CollectT) {
		...
		listNodes, err = stealthscale.ListNodes()
	...
	validateInitialConnection(t, stealthscale, expectedNodes)
	...
	assert.EventuallyWithT(t, func(ct *assert.CollectT) {
		listNodesAfterNewUserLogin, err = stealthscale.ListNodes()
	...
	require.EventuallyWithT(t, func(c *assert.CollectT) {
		nodeStore, err := stealthscale.DebugNodeStore()
	...
	assert.EventuallyWithT(t, func(ct *assert.CollectT) {
		currentNodes, err := stealthscale.ListNodes()
	...
	assert.EventuallyWithT(t, func(ct *assert.CollectT) {
		listNodesAfterLoggingBackIn, err = stealthscale.ListNodes()
...
// Prerequisites:
// - short TTL for the registration cache via STEALTHSCALE_TUNING_REGISTER_CACHE_EXPIRATION
// Scenario:
// - client starts a login process and gets initial AuthURL
// - time.sleep(STEALTHSCALE_TUNING_REGISTER_CACHE_EXPIRATION + 30 secs) waits for the cache to expire
...
func TestOIDCFollowUpUrl(t *testing.T) {
	...
	oidcMap := map[string]string{
		...
		"STEALTHSCALE_TUNING_REGISTER_CACHE_CLEANUP":    "10s",
		"STEALTHSCALE_TUNING_REGISTER_CACHE_EXPIRATION": "1m30s",
	}
	...
	// a little bit more than STEALTHSCALE_TUNING_REGISTER_CACHE_EXPIRATION (1m30s)
	//nolint:forbidigo // Intentional delay: must wait for real-time cache expiration (STEALTHSCALE_TUNING_REGISTER_CACHE_EXPIRATION=1m30s)
...
func TestOIDCMultipleOpenedLoginUrls(t *testing.T) {
	...
	u1, err := ts.LoginWithURL(stealthscale.GetEndpoint())
	require.NoError(t, err)

	u2, err := ts.LoginWithURL(stealthscale.GetEndpoint())
	require.NoError(t, err)
...
// credentials directly in stealthscale.
//
// This test validates the "same user relogin" behavior in stealthscale's OIDC authentication flow:
...
// It ensures that stealthscale properly handles session management while preserving device
...
func TestOIDCReloginSameNodeSameUser(t *testing.T) {
	...
	assert.EventuallyWithT(t, func(ct *assert.CollectT) {
		...
		initialNodes, err = stealthscale.ListNodes()
		assert.NoError(ct, err, "Failed to list nodes during initial validation")
	...
	assert.EventuallyWithT(t, func(ct *assert.CollectT) {
		finalNodes, err = stealthscale.ListNodes()
		assert.NoError(ct, err, "Failed to list nodes during final validation")
...
// when a tailscaled client restarts and reconnects to stealthscale.
//
// This test reproduces the bug reported in https://github.com/tomiwebpro/stealthscale/issues/2862
// where OIDC expiry was reset to 0001-01-01 00:00:00 after tailscaled restart.
//
// Test flow:
// 1. Node logs in with OIDC (gets 72h expiry)
// 2. Verify expiry is set correctly in stealthscale
...
// https://github.com/tomiwebpro/stealthscale/issues/2888
//
...
func TestOIDCACLPolicyOnJoin(t *testing.T) {
	...
	// Create stealthscale environment with ACL policy that allows OIDC user
			...
			&policyv2.Policy{
				...
				AutoApprovers: policyv2.AutoApproverPolicy{
					Routes: map[netip.Prefix]policyv2.AutoApprovers{
						netip.MustParsePrefix("10.33.0.0/24"): {usernameApprover("gateway@test.no"), usernameApprover("oidcuser@stealthscale.net"), usernameApprover("jane.doe@example.com")},
						netip.MustParsePrefix("10.44.0.0/24"): {usernameApprover("gateway@test.no"), usernameApprover("oidcuser@stealthscale.net"), usernameApprover("jane.doe@example.com")},
					},
				},
			},
		),
	...
	}, integrationutil.ScaledTimeout(10*time.Second), integrationutil.SlowPoll, "route advertisement should propagate to stealthscale")

	// Approve the advertised route
	_, err = stealthscale.ApproveRoutes(
		gatewayNodeID,
	...
	}, integrationutil.ScaledTimeout(10*time.Second), integrationutil.SlowPoll, "route approval should propagate to stealthscale")
	...
	loginURL, err := oidcClient.LoginWithURL(stealthscale.GetEndpoint())
	require.NoError(t, err)
	...
	// Additional validation: Verify nodes in stealthscale match expectations
	assert.EventuallyWithT(t, func(ct *assert.CollectT) {
		nodes, err := stealthscale.ListNodes()
		assert.NoError(ct, err)
		...
		users, err := stealthscale.ListUsers()
		assert.NoError(ct, err)
		...
		if oidcUserFound != nil {
			...
			assert.Equal(ct, "oidcuser@stealthscale.net", oidcUserFound.Email)
		}
	}, integrationutil.ScaledTimeout(10*time.Second), integrationutil.SlowPoll, "stealthscale should have correct users and nodes")
...
// https://github.com/tomiwebpro/stealthscale/issues/2896
//
...
// A stealthscale restart would fix it, indicating a state management issue.
	...
	err = scenario.CreateStealthScaleEnvWithLoginURL(
		...
		hsic.WithACLPolicy(
			&policyv2.Policy{
				...
				AutoApprovers: policyv2.AutoApproverPolicy{
					Routes: map[netip.Prefix]policyv2.AutoApprovers{
						netip.MustParsePrefix(advertiseRoute): {usernameApprover("user1@stealthscale.net")},
					},
				},
			},
		),
