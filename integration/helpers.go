...
// requireNoErrStealthScaleEnv validates that stealthscale environment creation succeeded.
// Provides specific error context for stealthscale environment setup failures.
func requireNoErrStealthScaleEnv(t *testing.T, err error) {
	t.Helper()
	require.NoError(t, err, "failed to create stealthscale environment")
}

// requireNoErrGetStealthScale validates that stealthscale server retrieval succeeded.
// Provides specific error context for stealthscale server access failures.
func requireNoErrGetStealthScale(t *testing.T, err error) {
	t.Helper()
	require.NoError(t, err, "failed to get stealthscale")
}
...
func validateInitialConnection(t *testing.T, stealthscale ControlServer, expectedNodes []types.NodeID) {
	t.Helper()

	requireAllClientsOnline(t, stealthscale, expectedNodes, true, "all clients should be connected after initial login", 120*time.Second)
	requireAllClientsNetInfoAndDERP(t, stealthscale, expectedNodes, "all clients should have NetInfo and DERP after initial login", 3*time.Minute)
}

// validateLogoutComplete performs comprehensive validation after client logout.
// Ensures all nodes are properly offline across all stealthscale systems,
// critical for validating clean logout state in relogin tests.
func validateLogoutComplete(t *testing.T, stealthscale ControlServer, expectedNodes []types.NodeID) {
	t.Helper()

	requireAllClientsOnline(t, stealthscale, expectedNodes, false, "all nodes should be offline after logout", 120*time.Second)
}
...
func validateReloginComplete(t *testing.T, stealthscale ControlServer, expectedNodes []types.NodeID) {
	t.Helper()

	requireAllClientsOnline(t, stealthscale, expectedNodes, true, "all clients should be connected after relogin", integrationutil.ScaledTimeout(120*time.Second))
	requireAllClientsNetInfoAndDERP(t, stealthscale, expectedNodes, "all clients should have NetInfo and DERP after relogin", integrationutil.ScaledTimeout(3*time.Minute))
}

// requireAllClientsOnline validates that all nodes are online/offline across all stealthscale systems
// requireAllClientsOnline verifies all expected nodes are in the specified online state across all systems.
func requireAllClientsOnline(t *testing.T, stealthscale ControlServer, expectedNodes []types.NodeID, expectedOnline bool, message string, timeout time.Duration) {
	t.Helper()
	...
	if expectedOnline {
		...
		requireAllClientsOnlineWithSingleTimeout(t, stealthscale, expectedNodes, expectedOnline, message, timeout)
	} else {
		// For offline validation, use staged approach with component-specific timeouts
		requireAllClientsOfflineStaged(t, stealthscale, expectedNodes)
	}
...
func requireAllClientsOnlineWithSingleTimeout(t *testing.T, stealthscale ControlServer, expectedNodes []types.NodeID, expectedOnline bool, message string, timeout time.Duration) {
	t.Helper()
	...
	require.EventuallyWithT(t, func(c *assert.CollectT) {
		// Get batcher state
		debugInfo, err := stealthscale.DebugBatcher()
		...
		// Get map responses
		mapResponses, err := stealthscale.GetAllMapReponses()
...
func requireAllClientsOfflineStaged(t *testing.T, stealthscale ControlServer, expectedNodes []types.NodeID) {
	t.Helper()
...
// [requireAllClientsOnline] by using [hsic.StealthScaleInContainer.DebugNodeStore] to get the database state.
//
//nolint:unparam // timeout is configurable for flexibility even though callers currently use same value
func requireAllClientsNetInfoAndDERP(t *testing.T, stealthscale ControlServer, expectedNodes []types.NodeID, message string, timeout time.Duration) {
	t.Helper()
...
// for validating OIDC integration flows in stealthscale authentication tests.
func oidcMockUser(username string, emailVerified bool) mockoidc.MockUser {
	return mockoidc.MockUser{
		Subject:           username,
		PreferredUsername: username,
		Email:             username + "@stealthscale.net",
		EmailVerified:     emailVerified,
	}
}

// GetUserByName retrieves a user by name from the stealthscale server.
// This is a common pattern used when creating preauth keys or managing users.
func GetUserByName(stealthscale ControlServer, username string) (*clientv1.User, error) {
...
func (s *Scenario) AddAndLoginClient(
	...
	user, err := GetUserByName(stealthscale, username)
	if err != nil {
	...
	err = newClient.Login(stealthscale.GetEndpoint(), authKey.Key)
	if err != nil {
...
func (s *Scenario) MustAddAndLoginClient(
	...
	client, err := s.AddAndLoginClient(t, username, version, stealthscale, tsOpts...)
	require.NoError(t, err)
