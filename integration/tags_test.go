...
func TestTagsAuthKeyWithTagRequestDifferentTag(t *testing.T) {
	...
	err = client.Login(stealthscale.GetEndpoint(), authKey.Key)
...
func TestTagsAuthKeyWithTagCannotAddViaCLI(t *testing.T) {
	...
	command := []string{
		"tailscale", "up",
		"--login-server=" + stealthscale.GetEndpoint(),
		"--authkey=" + authKey.Key,
...
// Setup:
//  1. Register with --auth-key AUTH_KEY_WITH_TAG
//  2. Assign ["tag:second"] via stealthscale CLI
...
func TestTagsAuthKeyWithTagAdminOverrideReauthPreserves(t *testing.T) {
	...
	// Step 2: Admin assigns different tags via stealthscale CLI
	err = stealthscale.SetNodeTags(nodeID, []string{"tag:second"})
	require.NoError(t, err)
...
// the same node/IP. Before the fix stealthscale consumes the new key but keeps the
...
// unchanged.
//
// https://github.com/tomiwebpro/stealthscale/issues/3370
func TestTagsReauthDifferentKeyRetagsNode(t *testing.T) {
	...
	err = client.Login(stealthscale.GetEndpoint(), key1.Key)
	require.NoError(t, err)
...
// Setup:
//  1. Register with --auth-key AUTH_KEY_WITH_TAG
//  2. Assign ["tag:valid-owned", "tag:second"] via stealthscale CLI
...
func TestTagsAuthKeyWithTagCLICannotModifyAdminTags(t *testing.T) {
	...
	// Step 2: Admin assigns multiple tags via stealthscale CLI
	err = stealthscale.SetNodeTags(nodeID, []string{"tag:valid-owned", "tag:second"})
	require.NoError(t, err)
...
// Setup:
//  1. Register with --auth-key AUTH_KEY_WITHOUT_TAG
//  2. Assign ["tag:valid-owned"] via stealthscale CLI
...
func TestTagsAuthKeyWithoutTagCLINoOpAfterAdminWithReset(t *testing.T) {
	...
	err = stealthscale.SetNodeTags(nodeID, []string{"tag:valid-owned"})
	require.NoError(t, err)
...
func TestTagsUserLoginOwnedTagAtRegistration(t *testing.T) {
	...
	loginURL, err := client.LoginWithURL(stealthscale.GetEndpoint())
	require.NoError(t, err)
	...
	// Register the node via stealthscale CLI
	err = scenario.runStealthScaleRegister(tagTestUser, body)
	require.NoError(t, err)
...
func TestTagsUserLoginNonExistentTagAtRegistration(t *testing.T) {
	...
	// Register the node via stealthscale CLI - this should fail due to non-existent tag
	err = scenario.runStealthScaleRegister(tagTestUser, body)
...
func TestTagsUserLoginUnownedTagAtRegistration(t *testing.T) {
	...
	_ = scenario.runStealthScaleRegister(tagTestUser, body)
...
func TestTagsAdminAPICannotSetNonExistentTag(t *testing.T) {
	...
	err = stealthscale.SetNodeTags(nodeID, []string{"tag:nonexistent"})
...
func TestTagsAdminAPICanSetUnownedTag(t *testing.T) {
	...
	err = stealthscale.SetNodeTags(nodeID, []string{"tag:valid-unowned"})
...
func TestTagsAdminAPICannotRemoveAllTags(t *testing.T) {
	...
	err = stealthscale.SetNodeTags(nodeID, []string{})
...
// 2. Admin changes tag to tag:bar via stealthscale CLI/API
...
func TestTagsIssue2978ReproTagReplacement(t *testing.T) {
	...
	// Use CreateStealthScaleEnvWithLoginURL for web auth flow
	err = scenario.CreateStealthScaleEnvWithLoginURL(
		[]tsic.Option{
			tsic.WithExtraLoginArgs([]string{"--advertise-tags=tag:valid-owned"}),
		},
...
func TestTagsAdminAPICannotSetInvalidFormat(t *testing.T) {
	...
	err = stealthscale.SetNodeTags(nodeID, []string{"invalid-no-prefix"})
...
// convert it to a user-owned node via "stealthscale auth register --auth-id <id> --user <user>".
...
func TestTagsAuthKeyConvertToUserViaCLIRegister(t *testing.T) {
	...
	// ShutdownAssertNoPanics in the defer catches any panics in stealthscale logs.
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		nodes, err := stealthscale.ListNodes()
		assert.NoError(c, err)
		assert.Len(c, nodes, 1)
...
//
// https://github.com/tomiwebpro/stealthscale/issues/3371
func TestTaggedNodeLogoutReloginSingleUseKeyOnline(t *testing.T) {
	...
	err = client.Login(stealthscale.GetEndpoint(), key2.Key)
	require.NoError(t, err,
...
func TestTaggedNodeLogoutReloginReusableKeyOnline(t *testing.T) {
	...
	err = client.Login(stealthscale.GetEndpoint(), key.Key)
	require.NoError(t, err)
...
//
// https://github.com/tomiwebpro/stealthscale/issues/3374
func TestTagsOIDCReauthAddOwnedTag(t *testing.T) {
	...
	// non-email usernames).
	owner := new(policyv2.Username(oidcUser + "@stealthscale.net"))
	...
	u, err := client.LoginWithURL(stealthscale.GetEndpoint())
	require.NoError(t, err)
