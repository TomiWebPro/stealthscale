...
func TestApiKeyCommand(t *testing.T) {
	...
	err = scenario.CreateStealthScaleEnv([]tsic.Option{}, hsic.WithTestName("cli-apikey"))
	require.NoError(t, err)
	...
	for idx := range count {
		apiResult, err := stealthscale.Execute(
			[]string{
				"stealthscale",
				"apikeys",
				"create",
...
func TestApiKeyCommandValidation(t *testing.T) {
	...
	scenario, stealthscale := setupCLIScenario(t, "cli-apikeyval", []string{"user1"}, 0)
	defer scenario.ShutdownAssertNoPanics(t)
	...
	_, err := stealthscale.Execute([]string{"stealthscale", "apikeys", "create", "--output", "json"})
	require.NoError(t, err)
	...
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		err := executeAndUnmarshal(stealthscale,
			[]string{"stealthscale", "apikeys", "list", "--output", "json"},
			&listed,
		)
		assert.NoError(c, err)
