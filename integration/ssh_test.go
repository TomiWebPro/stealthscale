...
// findSSHCheckAuthID polls stealthscale container logs for the SSH action auth-id.
// The SSH action handler logs "SSH action follow-up" with the auth_id on the
// follow-up request (where auth_id is non-empty).
func findSSHCheckAuthID(t *testing.T, stealthscale ControlServer) string {
	t.Helper()

	var authID string

	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		_, stderr, err := stealthscale.ReadLog()
		...
		assert.NotEmpty(c, authID, "auth-id not found in stealthscale logs")
	}, integrationutil.ScaledTimeout(10*time.Second), integrationutil.SlowPoll, "waiting for SSH check auth-id in stealthscale logs")
...
// findNewSSHCheckAuthID polls stealthscale logs for an SSH check auth-id
...
func findNewSSHCheckAuthID(
	...
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		...
		assert.NotEmpty(c, authID, "new auth-id not found in stealthscale logs")
...
func TestSSHOneUserToOneCheckModeCLI(t *testing.T) {
	...
	for _, client := range user1Clients {
		for _, peer := range allClients {
			...
			// Find the auth-id from stealthscale logs
			authID := findSSHCheckAuthID(t, stealthscale)

			// Approve via CLI
			_, err := stealthscale.Execute(
				[]string{
					"stealthscale", "auth", "approve",
					"--auth-id", authID,
				},
			)
			require.NoError(t, err)
...
func TestSSHOneUserToOneCheckModeOIDC(t *testing.T) {
	...
	for _, client := range user1Clients {
		for _, peer := range allClients {
			...
			authURL := stealthscale.GetEndpoint() + "/auth/" + authID
...
func TestSSHCheckModeUnapprovedTimeout(t *testing.T) {
	...
	err = scenario.CreateStealthScaleEnv(
		...
		hsic.WithConfigEnv(map[string]string{
			"STEALTHSCALE_TUNING_REGISTER_CACHE_EXPIRATION": "15s",
			"STEALTHSCALE_TUNING_REGISTER_CACHE_CLEANUP":    "5s",
		}),
	)
	require.NoError(t, err)
	...
	for _, client := range user1Clients {
		for _, peer := range allClients {
			...
			_ = findSSHCheckAuthID(t, stealthscale)
...
func TestSSHCheckModeCheckPeriodCLI(t *testing.T) {
	...
	for _, client := range user1Clients {
		for _, peer := range allClients {
			...
			sshResult := doSSHCheck(t, client, peer)
			firstAuthID := findSSHCheckAuthID(t, stealthscale)
			...
			secondAuthID := findNewSSHCheckAuthID(t, stealthscale, firstAuthID)
...
// https://github.com/tomiwebpro/stealthscale/issues/3305 with a real client: an SSH
...
func TestSSHCheckModeSessionLossReDelegates(t *testing.T) {
	...
	for _, client := range user1Clients {
		for _, peer := range allClients {
			...
			err := stealthscale.Restart()
			require.NoError(t, err, "restarting stealthscale should succeed")
...
// TestSSHCheckModeNegativeCLI verifies that `stealthscale auth reject`
...
// so user1@stealthscale.net can SSH as local user "user1".
				...
				SSHs: []policyv2.SSH{
					{
						...
						Users: []policyv2.SSHUser{"localpart:*@stealthscale.net"},
					},
				},
			},
			testFn: func(t *testing.T, scenario *Scenario) {
				...
				// user1 can SSH to user2's nodes as "user1" (localpart of user1@stealthscale.net)
				for _, client := range user1Clients {
					for _, peer := range user2Clients {
				...
				// user2 can SSH to user1's nodes as "user2" (localpart of user2@stealthscale.net)
				for _, client := range user2Clients {
					for _, peer := range user1Clients {
