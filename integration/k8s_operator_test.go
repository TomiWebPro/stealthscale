...
import (
	...
	"github.com/tomiwebpro/stealthscale/integration/k3sic"
...
// into a real k3s cluster via its Helm chart and pointed at an in-test StealthScale,
...
//
// The operator targets StealthScale over plain HTTP by IP (hsic.WithoutTLS +
// GetIPEndpoint), so the operator and proxy pods need no CA and no DNS entry for
// StealthScale. See integration/k3sic/tls-ca-baking.md for the TLS variant.
//
...
func TestK8sOperator(t *testing.T) {
	...
	clientID, clientSecret, err := stealthscale.CreateOAuthClient(
		t.Context(),
	...
	// references StealthScale by hostname; teach CoreDNS to resolve it so the proxy
	// pods can connect to DERP and get a data path to nodes outside the cluster.
	hsIP := stealthscale.GetIPInNetwork(scenario.Networks()[0])
	require.NoError(t, k3s.ConfigureCoreDNSHost(stealthscale.GetHostname(), hsIP))
	...
	loginServer := stealthscale.GetIPEndpoint()
	...
	t.Run("ingress-service-reachable-from-tailnet", func(t *testing.T) {
		...
		// its IP from StealthScale rather than the Service's LoadBalancer status,
		// which the operator does not populate against StealthScale.
		var svcIP string
