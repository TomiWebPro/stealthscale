...
func TestResolveMagicDNS(t *testing.T) {
	...
	err = scenario.CreateStealthScaleEnv([]tsic.Option{}, hsic.WithTestName("magicdns"))
	...
	for _, client := range allClients {
		for _, peer := range allClients {
			...
			assert.Equal(t, peer.Hostname()+".stealthscale.net.", peerFQDN)
...
func TestResolveMagicDNSExtraRecordsPath(t *testing.T) {
	...
	err = scenario.CreateStealthScaleEnv([]tsic.Option{
		...
		hsic.WithConfigEnv(map[string]string{
			...
			"STEALTHSCALE_DNS_NAMESERVERS_GLOBAL": "",
			"STEALTHSCALE_DNS_EXTRA_RECORDS_PATH": erPath,
		}),
