...
// point it at an in-test StealthScale. k3s bundles kubectl and we run helm inside
// the container through Execute, so the host dev shell needs no kube tooling.
//
// The operator is pointed at StealthScale over plain HTTP (see
...
// StealthScale with a private CA, see tls-ca-baking.md.
//
...
// from the Tailscale minor StealthScale tracks via capver (e.g. "v1.98"). The
// operator shares the Tailscale release train, so this keeps the images and the
// Helm chart in lockstep with the client versions StealthScale is tested against,
...
// value the operator needs to target StealthScale instead of the Tailscale SaaS
...
// The operator targets StealthScale's control plane by IP, but the embedded DERP map
// references StealthScale by hostname; without this, the proxy pods cannot resolve
