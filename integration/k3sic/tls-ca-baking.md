# Running the operator against a private-CA TLS StealthScale

`TestK8sOperator` points the Tailscale Kubernetes operator at an **HTTP**
StealthScale (`hsic.WithoutTLS`, `loginServer = http://<ip>:<port>`). That keeps
...
This note records how to run the same test against a **TLS** StealthScale serving a
...
1. Serve StealthScale with TLS (the `hsic` default) and grab `stealthscale.GetCert()`.
...
2. Bake the CA into derived operator and proxy images. For each of
   ...
   ```Dockerfile
   FROM <base>
   COPY stealthscale-ca.crt /usr/local/share/ca-certificates/stealthscale-ca.crt
   ...
   Tag the derived images `stealthscale.local/...:<tag>-ca` and run them with
   `imagePullPolicy: Never` (the `stealthscale.local/` prefix is never resolved by
   a registry).
...
4. Pods resolve StealthScale through CoreDNS, not the container's `/etc/hosts`, and
   the cert.s only SAN is the StealthScale hostname (dialing by IP fails TLS
   verification). Install a `coredns-custom` ConfigMap mapping the hostname to
   the StealthScale IP and gate on the `kube-dns` Service having ready endpoints
