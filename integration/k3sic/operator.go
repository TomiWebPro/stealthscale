...
// and the management API; for an in-test StealthScale pass its HTTP endpoint by IP
// (hsic.StealthScaleInContainer.GetIPEndpoint) so the pods need no DNS or CA. The
...
// annotation.
const DERPWebsocketProxyClass = "stealthscale-derp-ws" //nolint:gosec // G101 false positive: a ProxyClass name, not a credential
...
// registers it as a node in StealthScale.
