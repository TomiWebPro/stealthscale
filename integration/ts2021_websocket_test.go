...
//
// These two tests guard that path against real stealthscale:
//
...
//     completes the Noise handshake with stealthscale over the WebSocket.
//
...
func TestTS2021WebSocketGET(t *testing.T) {
	...
	err = scenario.CreateStealthScaleEnv([]tsic.Option{}, hsic.WithTestName("ts2021ws"))
	requireNoErrStealthScaleEnv(t, err)

	stealthscale, err := scenario.StealthScale()
	requireNoErrGetStealthScale(t, err)

	conn, err := dialTS2021WebSocket(t, stealthscale.GetEndpoint(), stealthscale.GetCert())
...
// inside a Node container against real stealthscale, next to normal Tailscale
...
func TestTS2021WASMClientUnderNode(t *testing.T) {
	...
	// :8080. Run stealthscale without TLS, matching the http:// setup in the issue.
...
// stealthscale by hostname. It idles; the test execs the client on demand.
...
// fetchServerNoiseKey retrieves stealthscale's Noise public key from /key, the same
