...
func executeAndUnmarshal[T any](stealthscale ControlServer, command []string, result T) error {
	str, err := stealthscale.Execute(command)
	if err != nil {
		return err
	}
...
func assertJSONRoundtrip[T any](t require.TestingT, stealthscale ControlServer, command []string) T {
	var first T

	err := executeAndUnmarshal(stealthscale, command, &first)
...
// creates the stealthscale environment and returns the running scenario and its
// control server. It removes the repeated NewScenario/CreateStealthScaleEnv/
// StealthScale boilerplate shared by the CLI tests. Callers still defer
...
func setupCLIScenario(t *testing.T, testName string, users []string, nodesPerUser int) (*Scenario, ControlServer) {
	...
	err = scenario.CreateStealthScaleEnv([]tsic.Option{}, hsic.WithTestName(testName))
	require.NoError(t, err)

	stealthscale, err := scenario.StealthScale()
	require.NoError(t, err)

	return scenario, stealthscale
}
