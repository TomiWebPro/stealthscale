...
func TestPingAllByIP(t *testing.T) {
	...
	// Get stealthscale instance for batcher debug check
	stealthscale, err := scenario.StealthScale()
	require.NoError(t, err)

	// Test our DebugBatcher functionality
	t.Logf("Testing DebugBatcher functionality...")
	requireAllClientsOnline(t, stealthscale, expectedNodes, true, "all clients should be connected to the batcher", integrationutil.ScaledTimeout(30*time.Second))
...
func TestPingAllByIPPublicDERP(t *testing.T) {
	...
	err = scenario.CreateStealthScaleEnv(
		...
		// infrastructure. TLS is disabled because the stealthscale
...
func testEphemeralWithOptions(t *testing.T, opts ...hsic.Option) {
	...
	stealthscale, err := scenario.StealthScale(opts...)
	requireNoErrStealthScaleEnv(t, err)
...
func TestEphemeral2006DeletedTooQuickly(t *testing.T) {
	...
	stealthscale, err := scenario.StealthScale(
		...
		hsic.WithConfigEnv(map[string]string{
			"STEALTHSCALE_EPHEMERAL_NODE_INACTIVITY_TIMEOUT": "1m6s",
		}),
	)
	requireNoErrStealthScaleEnv(t, err)
	...
	assert.EventuallyWithT(t, func(ct *assert.CollectT) {
		for _, userName := range spec.Users {
			nodes, err := stealthscale.ListNodes(userName)
			assert.NoError(ct, err)
...
func TestPingAllByHostname(t *testing.T) {
	...
	err = scenario.CreateStealthScaleEnv([]tsic.Option{}, hsic.WithTestName("pingallbyname"))
	requireNoErrStealthScaleEnv(t, err)
...
func TestTaildrop(t *testing.T) {
	...
	for i, version := range MustTestVersions {
		...
		err = client.Login(stealthscale.GetEndpoint(), user1Key.Key)
		require.NoError(t, err)
	...
	for i, version := range MustTestVersions {
		...
		err = client.Login(stealthscale.GetEndpoint(), user2Key.Key)
		require.NoError(t, err)
	...
	err = taggedClient.Login(stealthscale.GetEndpoint(), taggedKey.Key)
	require.NoError(t, err)
...
func TestUpdateHostnameFromClient(t *testing.T) {
	...
	err = scenario.CreateStealthScaleEnv([]tsic.Option{}, hsic.WithTestName("updatehostname"))
	requireNoErrStealthScaleEnv(t, err)
	...
	assert.EventuallyWithT(t, func(ct *assert.CollectT) {
		...
		for _, node := range nodes {
			nodeID := node.Id
			expectedDNSNames[nodeID] = fmt.Sprintf("%s-givenname.stealthscale.net.", node.Id)
		}
...
func TestExpireNode(t *testing.T) {
	...
	err = scenario.CreateStealthScaleEnv([]tsic.Option{}, hsic.WithTestName("expirenode"))
	requireNoErrStealthScaleEnv(t, err)
	...
	// TODO(kradalby): This is StealthScale specific and would not play nicely
	// with other implementations of the [ControlServer] interface
	result, err := stealthscale.Execute([]string{
		"stealthscale", "nodes", "expire", "--identifier", "1", "--output", "json",
	})
	require.NoError(t, err)
...
func TestSetNodeExpiryInFuture(t *testing.T) {
	...
	err = scenario.CreateStealthScaleEnv([]tsic.Option{}, hsic.WithTestName("expirenodefuture"))
	requireNoErrStealthScaleEnv(t, err)
	...
	result, err := stealthscale.Execute(
		[]string{
			"stealthscale", "nodes", "expire",
			"--identifier", "1",
			"--output", "json",
...
func TestDisableNodeExpiry(t *testing.T) {
	...
	err = scenario.CreateStealthScaleEnv([]tsic.Option{}, hsic.WithTestName("disableexpiry"))
	requireNoErrStealthScaleEnv(t, err)
...
func TestNodeOnlineStatus(t *testing.T) {
	...
	err = scenario.CreateStealthScaleEnv([]tsic.Option{}, hsic.WithTestName("online"))
	requireNoErrStealthScaleEnv(t, err)
	...
	for {
		...
		assert.EventuallyWithT(t, func(ct *assert.CollectT) {
			result, err := stealthscale.Execute([]string{
				"stealthscale", "nodes", "list", "--output", "json",
			})
			assert.NoError(ct, err)
			...
			// Verify that stealthscale reports the nodes as online
			for _, node := range nodes {
				// All nodes should be online
				assert.Truef(
					ct,
					node.Online,
					"expected %s to have online status in StealthScale, marked as offline %s after start",
					node.Name,
					time.Since(start),
				)
			}
...
func TestPingAllByIPManyUpDown(t *testing.T) {
	...
	// Get stealthscale instance for batcher debug checks
	stealthscale, err := scenario.StealthScale()
	require.NoError(t, err)
	...
	requireAllClientsOnline(t, stealthscale, expectedNodes, true, "all clients should be connected to batcher", integrationutil.ScaledTimeout(30*time.Second))
	...
	for run := range 3 {
		...
		requireAllClientsOnline(t, stealthscale, expectedNodes, false, fmt.Sprintf("Run %d: all nodes should be offline after Down()", run+1), integrationutil.ScaledTimeout(120*time.Second))
		...
		requireAllClientsOnline(t, stealthscale, expectedNodes, true, fmt.Sprintf("Run %d: all nodes should be reconnected after Up()", run+1), integrationutil.ScaledTimeout(120*time.Second))
		...
		requireAllClientsOnline(t, stealthscale, expectedNodes, true, fmt.Sprintf("Run %d: all systems should show nodes online after reconnection", run+1), integrationutil.ScaledTimeout(60*time.Second))
