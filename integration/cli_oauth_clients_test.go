...
// TestOAuthClientCommand exercises the `stealthscale oauth-clients` CLI end to end:
...
func TestOAuthClientCommand(t *testing.T) {
	...
	err = scenario.CreateStealthScaleEnv([]tsic.Option{}, hsic.WithTestName("cli-oauthclient"))
	require.NoError(t, err)
	...
	createOut, err := stealthscale.Execute([]string{
		"stealthscale", "oauth-clients", "create",
		"--scope", "auth_keys",
		"--scope", "devices:core",
	...
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		err := executeAndUnmarshal(stealthscale,
			[]string{"stealthscale", "oauth-clients", "list", "--output", "json"}, &listed)
	...
	// Delete it.
	_, err = stealthscale.Execute([]string{"stealthscale", "oauth-clients", "delete", "--id", created.ID})
	require.NoError(t, err)
	...
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		err := executeAndUnmarshal(stealthscale,
			[]string{"stealthscale", "oauth-clients", "list", "--output", "json"}, &afterDelete)
		assert.NoError(c, err)
...
func TestOAuthClientCommandValidation(t *testing.T) {
	...
	err = scenario.CreateStealthScaleEnv([]tsic.Option{}, hsic.WithTestName("cli-oauthclientval"))
	require.NoError(t, err)
