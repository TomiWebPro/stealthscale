...
func TestUserCommand(t *testing.T) {
	...
	err = scenario.CreateStealthScaleEnv([]tsic.Option{}, hsic.WithTestName("cli-user"))
	require.NoError(t, err)
	...
	deleteResult, err := stealthscale.Execute(
		[]string{
			"stealthscale",
			"users",
			"destroy",
...
// TestUserCreateCommand exercises `stealthscale users create` with all of its
...
func TestUserCreateCommand(t *testing.T) {
	...
	scenario, stealthscale := setupCLIScenario(t, "cli-usercreate", []string{"existing"}, 1)
	defer scenario.ShutdownAssertNoPanics(t)
	...
	created := assertJSONRoundtrip[*clientv1.User](t, stealthscale, []string{
		"stealthscale",
		"users",
		"create",
		"cli-created",
...
func TestUserCommandValidation(t *testing.T) {
	...
	scenario, stealthscale := setupCLIScenario(t, "cli-userval", []string{"user1"}, 0)
	defer scenario.ShutdownAssertNoPanics(t)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			out, err := stealthscale.Execute(append([]string{"stealthscale"}, tt.args...))
