...
func TestDERPServerScenario(t *testing.T) {
	...
	derpServerScenario(t, spec, "derp-tcp", false, func(scenario *Scenario) {
		...
		hsServer, err := scenario.StealthScale()
...
func derpServerScenario(
	...
	err = scenario.CreateStealthScaleEnv(
		...
		hsic.WithConfigEnv(map[string]string{
			"STEALTHSCALE_DERP_AUTO_UPDATE_ENABLED":   "true",
			"STEALTHSCALE_DERP_UPDATE_FREQUENCY":      "10s",
			"STEALTHSCALE_LISTEN_ADDR":                "0.0.0.0:443",
			"STEALTHSCALE_DERP_SERVER_VERIFY_CLIENTS": "true",
		}),
	)
	...
	for _, client := range allClients {
		assert.EventuallyWithT(t, func(ct *assert.CollectT) {
			...
			for _, health := range status.Health {
				...
				assert.NotContains(ct, health, "could not connect to the 'StealthScale Embedded DERP' relay server.",
					"Client %s should be connected to StealthScale Embedded DERP", client.Hostname())
			}
	...
	for _, client := range allClients {
		assert.EventuallyWithT(t, func(ct *assert.CollectT) {
			...
			for _, health := range status.Health {
				...
				assert.NotContains(ct, health, "could not connect to the 'StealthScale Embedded DERP' relay server.",
					"Client %s should be connected to StealthScale Embedded DERP after first run", client.Hostname())
			}
		}, integrationutil.StatusReadyTimeout, 2*time.Second)
	}
	...
	for _, client := range allClients {
		assert.EventuallyWithT(t, func(ct *assert.CollectT) {
			...
			for _, health := range status.Health {
				...
				assert.NotContains(ct, health, "could not connect to the 'StealthScale Embedded DERP' relay server.",
					"Client %s should be connected to StealthScale Embedded DERP after second run", client.Hostname())
			}
		}, integrationutil.StatusReadyTimeout, 2*time.Second)
	}
