...
func TestPreAuthKeyCommand(t *testing.T) {
	...
	err = scenario.CreateStealthScaleEnv([]tsic.Option{}, hsic.WithTestName("clipak"))
	require.NoError(t, err)
	...
	// There is one key created by [Scenario.CreateStealthScaleEnv]
...
func TestPreAuthKeyCommandWithoutExpiry(t *testing.T) {
	...
	err = scenario.CreateStealthScaleEnv([]tsic.Option{}, hsic.WithTestName("clipaknaexp"))
	require.NoError(t, err)
...
func TestPreAuthKeyCommandReusableEphemeral(t *testing.T) {
	...
	err = scenario.CreateStealthScaleEnv([]tsic.Option{}, hsic.WithTestName("clipakresueeph"))
	require.NoError(t, err)
...
func TestPreAuthKeyDeleteCommand(t *testing.T) {
	...
	scenario, stealthscale := setupCLIScenario(t, "cli-pakdelete", []string{"user1"}, 0)
	defer scenario.ShutdownAssertNoPanics(t)

	// Create a key to delete.
	created := assertJSONRoundtrip[*clientv1.PreAuthKey](t, stealthscale, []string{
		"stealthscale",
		"preauthkeys",
		"--user", "1",
		"create",
	...
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		...
		err := executeAndUnmarshal(stealthscale,
			[]string{"stealthscale", "preauthkeys", "list", "--output", "json"},
			&listed,
		)
		assert.NoError(c, err)
...
func TestPreAuthKeyCommandValidation(t *testing.T) {
	...
	scenario, stealthscale := setupCLIScenario(t, "cli-pakval", []string{"user1"}, 0)
	defer scenario.ShutdownAssertNoPanics(t)
