...
// integration testing with stealthscale.
//
...
// so verification is done externally via stealthscale API and peer connectivity.
package tsric
...
func getPrebuiltImage() string {
	return os.Getenv("STEALTHSCALE_INTEGRATION_TAILSCALE_RS_IMAGE")
}
...
// WithStealthScaleURL sets the stealthscale control server URL.
func WithStealthScaleURL(url string) Option {
	return func(t *TailscaleRustInContainer) {
		t.stealthscaleURL = url
	}
}
...
func (t *TailscaleRustInContainer) buildEntrypoint() []string {
	...
	commands = append(
		commands,
		fmt.Sprintf(`export TS_CONTROL_URL=%q`, t.stealthscaleURL),
...
func New(
	...
	if t.stealthscaleURL == "" {
		return nil, errors.New("tsric: no stealthscale URL set") //nolint:err113
	}
