...
func TestAuthKeyLogoutAndReloginSameUser(t *testing.T) {
	...
	for _, https := range []bool{true, false} {
		t.Run(fmt.Sprintf("with-https-%t", https), func(t *testing.T) {
			...
			err = scenario.CreateStealthScaleEnv([]tsic.Option{}, opts...)
			...
			requireAllClientsOnline(t, stealthscale, expectedNodes, true, "all clients should be connected", integrationutil.ScaledTimeout(120*time.Second))

			// Validate that all nodes have [tailcfg.NetInfo] and DERP servers before logout
			requireAllClientsNetInfoAndDERP(t, stealthscale, expectedNodes, "all clients should have NetInfo and DERP before logout", 3*time.Minute)
			...
			requireAllClientsOnline(t, stealthscale, expectedNodes, false, "all nodes should have logged out", integrationutil.ScaledTimeout(120*time.Second))
			...
			// https://github.com/tomiwebpro/stealthscale/issues/2164
			if !https {
			...
			for _, userName := range spec.Users {
				...
				err = scenario.RunTailscaleUp(userName, stealthscale.GetEndpoint(), key.Key)
				if err != nil {
			...
			requireAllClientsOnline(t, stealthscale, expectedNodes, true, "all clients should be connected to batcher", integrationutil.ScaledTimeout(120*time.Second))
			...
			requireAllClientsNetInfoAndDERP(t, stealthscale, expectedNodes, "all clients should have NetInfo and DERP after reconnection", 3*time.Minute)
...
func TestAuthKeyLogoutAndReloginNewUser(t *testing.T) {
	...
	err = scenario.CreateStealthScaleEnv([]tsic.Option{},
	...
	requireAllClientsOnline(t, stealthscale, expectedNodes, true, "all clients should be connected after initial login", integrationutil.ScaledTimeout(120*time.Second))
	...
	requireAllClientsOnline(t, stealthscale, expectedNodes, false, "all nodes should be offline after logout", integrationutil.ScaledTimeout(120*time.Second))
	...
	assert.EventuallyWithT(t, func(ct *assert.CollectT) {
		...
		user1Nodes, err = stealthscale.ListNodes("user1")
	...
	requireAllClientsOnline(t, stealthscale, expectedUser1Nodes, true, "all user1 nodes should be connected after relogin", integrationutil.ScaledTimeout(120*time.Second))
	requireAllClientsNetInfoAndDERP(t, stealthscale, expectedUser1Nodes, "all user1 nodes should have NetInfo and DERP after relogin", 3*time.Minute)
	...
	assert.EventuallyWithT(t, func(ct *assert.CollectT) {
		...
		user2Nodes, err = stealthscale.ListNodes("user2")
...
func TestAuthKeyDeleteKey(t *testing.T) {
	...
	err = scenario.CreateStealthScaleEnv([]tsic.Option{}, hsic.WithTestName("delkey"))
	requireNoErrStealthScaleEnv(t, err)
	...
	err = client.Login(stealthscale.GetEndpoint(), authKeyString)
	require.NoError(t, err)
	...
	requireAllClientsOnline(t, stealthscale, []types.NodeID{types.NodeID(nodeID)}, true, "node should be online initially", integrationutil.ScaledTimeout(120*time.Second))

	// DELETE the pre-auth key using the API
	t.Logf("Deleting pre-auth key ID %d using API", authKeyID)

	err = stealthscale.DeleteAuthKey(authKeyID)
	require.NoError(t, err)
	...
	requireAllClientsOnline(t, stealthscale, []types.NodeID{types.NodeID(nodeID)}, true, "node should reconnect after restart despite deleted key", integrationutil.ScaledTimeout(120*time.Second))
...
func TestAuthKeyLogoutAndReloginRoutesPreserved(t *testing.T) {
	...
	err = scenario.RunTailscaleUp(user, stealthscale.GetEndpoint(), key.Key)
	require.NoError(t, err)
