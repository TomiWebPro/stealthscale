...
var (
	...
	errInvalidTailscaleImageFormat     = errors.New("invalid STEALTHSCALE_INTEGRATION_TAILSCALE_IMAGE format, expected repository:tag")
	errTailscaleImageRequiredInCI      = errors.New("STEALTHSCALE_INTEGRATION_TAILSCALE_IMAGE must be set in CI for HEAD version")
...
// WithStealthScaleName set the name of the stealthscale instance,
// mostly useful in combination with TLS and WithCACert.
func WithStealthScaleName(hsName string) Option {
	return func(tsic *TailscaleInContainer) {
		tsic.stealthscaleHostname = hsName
	}
}
...
// counterpart to [hsic.WithoutTLS]: a StealthScale serving its embedded DERP without
...
func New(
	...
	case VersionHead:
		...
		prebuiltImage := os.Getenv("STEALTHSCALE_INTEGRATION_TAILSCALE_IMAGE")
