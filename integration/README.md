...
StealthScale.s integration tests start a real StealthScale server and run
...
- **`scenario.go`** — `Scenario` orchestrates a test environment: a
  StealthScale server, one or more users, and a collection of Tailscale
  clients. `NewScenario(spec)` returns a ready-to-use environment.
- **`hsic/`** — "StealthScale Integration Container": wraps a StealthScale
...
Tests compose these pieces via `ScenarioSpec` and `CreateStealthScaleEnv`
...
The canonical setup creates users, clients, and the StealthScale server in
one shot:
...
var err error
assert.EventuallyWithT(t, func(c *assert.CollectT) {
    nodes, err = stealthscale.ListNodes()   // = not :=
    assert.NoError(c, err)
...
The order of `stealthscale.ListNodes()` is not stable. Tests that index
...
func TestRouteAdvertisementBasic(t *testing.T) {
    ...
    err = scenario.CreateStealthScaleEnv([]tsic.Option{}, hsic.WithTestName("route"))
    require.NoError(t, err)
    ...
    // --- Eventually: stealthscale should see the announced route ---
    var nodes []*v1.Node
    assert.EventuallyWithT(t, func(c *assert.CollectT) {
    ...
    // --- Blocking: approve the route via stealthscale CLI ---
    ...
    _, err = stealthscale.ApproveRoutes(routerNode.GetId(), []string{"10.33.0.0/16"})
    require.NoError(t, err)
