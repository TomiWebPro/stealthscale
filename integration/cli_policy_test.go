...
// TestPolicyCheckCommand exercises `stealthscale policy check` across the
...
// via `STEALTHSCALE_POLICY_MODE`. The two users + two nodes give the tests
...
func TestPolicyCheckCommand(t *testing.T) {
	...
	modes := []string{"file", "database"} //nolint:goconst // axis labels match STEALTHSCALE_POLICY_MODE values
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			err = scenario.CreateStealthScaleEnv(
				...
				hsic.WithConfigEnv(map[string]string{
					"STEALTHSCALE_POLICY_MODE": tt.policyMode, //nolint:goconst // env var name from hscontrol/types/config.go
				}),
			)
			require.NoError(t, err)
			...
			policyFilePath := "/etc/stealthscale/policy.json" //nolint:goconst // standard stealthscale policy path
			err = stealthscale.WriteFile(policyFilePath, pBytes)
			require.NoError(t, err)

			cmd := []string{"stealthscale", "policy", "check", "-f", policyFilePath} //nolint:goconst // CLI invocation
			if tt.bypass {
			...
			stdout, err := stealthscale.Execute(cmd)
...
// TestSSHTestsRejectFailingPolicy asserts `stealthscale policy set` rejects
...
func TestSSHTestsRejectFailingPolicy(t *testing.T) {
	...
	err = scenario.CreateStealthScaleEnv(
		...
		hsic.WithConfigEnv(map[string]string{
			"STEALTHSCALE_POLICY_MODE": types.PolicyModeDB,
		}),
	)
	require.NoError(t, err)
	...
	const (
		goodPath = "/etc/stealthscale/policy-good.json"
		badPath  = "/etc/stealthscale/policy-bad.json"
	)

	require.NoError(t, stealthscale.WriteFile(goodPath, goodBytes))
	require.NoError(t, stealthscale.WriteFile(badPath, badBytes))

	// Establish the good policy as the live policy.
	_, err = stealthscale.Execute([]string{
		"stealthscale", "policy", "set", "-f", goodPath,
	})
	require.NoError(t, err, "setting the good policy must succeed")

	// Confirm the server returns the good policy.
	stdoutBefore, err := stealthscale.Execute([]string{
		"stealthscale", "policy", "get",
	})
	require.NoError(t, err)
	...
	// non-zero.
	_, err = stealthscale.Execute([]string{
		"stealthscale", "policy", "set", "-f", badPath,
	})
	...
	stdoutAfter, err := stealthscale.Execute([]string{
		"stealthscale", "policy", "get",
	})
	require.NoError(t, err)
...
func TestPolicyCommand(t *testing.T) {
	...
	err = scenario.CreateStealthScaleEnv(
		...
		hsic.WithConfigEnv(map[string]string{
			"STEALTHSCALE_POLICY_MODE": "database", // test sets/gets policy via CLI
		}),
	)
	require.NoError(t, err)
	...
	policyFilePath := "/etc/stealthscale/policy.json"
...
func TestPolicyBrokenConfigCommand(t *testing.T) {
	...
	err = scenario.CreateStealthScaleEnv(
		...
		hsic.WithConfigEnv(map[string]string{
			"STEALTHSCALE_POLICY_MODE": "database", // test sets invalid policy via CLI
		}),
	)
	require.NoError(t, err)
