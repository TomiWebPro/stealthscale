...
func TestHASubnetRouterFailover(t *testing.T) {
	...
	validatePrimaryRoutes := func(t *testing.T, stealthscale ControlServer, expectedRoutes *types.DebugRoutes, message string) {
		t.Helper()
		assert.EventuallyWithT(t, func(c *assert.CollectT) {
			primaryRoutesState, err := stealthscale.PrimaryRoutes()
			assert.NoError(c, err)
	...
	validatePrimaryRoutes(t, stealthscale, &types.DebugRoutes{
	...
	_, err = stealthscale.ApproveRoutes(mustParseID(MustFindNode(subRouter3.Hostname(), nodes).Id), []netip.Prefix{})
	...
	_, err = stealthscale.ApproveRoutes(mustParseID(MustFindNode(subRouter1.Hostname(), nodes).Id), []netip.Prefix{})
...
// https://github.com/tomiwebpro/stealthscale/issues/1604
...
// TestExitRoutesWithAutogroupInternetACL reproduces juanfont/stealthscale#3212.
...
// routes-b17/b18 in tscap). The bug was that stealthscale stripped
...
func TestSubnetRouterMultiNetwork(t *testing.T) {
	...
	err = scenario.CreateStealthScaleEnv([]tsic.Option{tsic.WithAcceptRoutes()},
...
func TestSubnetRouterMultiNetworkExitNode(t *testing.T) {
	...
	_, err = stealthscale.ApproveRoutes(mustParseID(nodes[0].Id), []netip.Prefix{tsaddr.AllIPv4(), tsaddr.AllIPv6(), *route})
	require.NoError(t, err)
		...
		for _, polMode := range []types.PolicyMode{types.PolicyModeDB, types.PolicyModeFile} {
			for _, advertiseDuringUp := range []bool{false, true} {
				...
				t.Run(name, func(t *testing.T) {
					...
					err = scenario.createStealthScaleEnvWithTags(tt.withURL, tsOpts, preAuthKeyTags, webauthTagUser,
						opts...,
					)
					requireNoErrStealthScaleEnv(t, err)
					...
					err = stealthscale.SetPolicy(pol)
					require.NoError(t, err)
					...
					if tt.withURL {
						u, err := routerUsernet1.LoginWithURL(stealthscale.GetEndpoint())
						require.NoError(t, err)

						body, err := doLoginURL(routerUsernet1.Hostname(), u)
						require.NoError(t, err)

						err = scenario.runStealthScaleRegister("user1", body)
						require.NoError(t, err)
						...
						err = routerUsernet1.Login(stealthscale.GetEndpoint(), pak.Key)
						require.NoError(t, err)
					}
					// extra creation end.
...
func TestSubnetRouteACLFiltering(t *testing.T) {
	...
	require.NoError(t, stealthscale.SetPolicy(aclPolicy))
	...
	assert.EventuallyWithT(t, func(ct *assert.CollectT) {
		...
		nodes, err := stealthscale.NodesByUser()
		assert.NoError(ct, err)
		assert.Len(ct, nodes, 2)
...
func TestGrantViaSubnetSteering(t *testing.T) {
	...
	// Get the subnet for usernet1 before creating stealthscale
	...
	// Create users on stealthscale server.
	...
	require.NoError(t, err)
	err = routerA.Login(stealthscale.GetEndpoint(), pakRouterA.Key)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	err = routerB.Login(stealthscale.GetEndpoint(), pakRouterB.Key)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	err = clientA.Login(stealthscale.GetEndpoint(), pakClientA.Key)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	err = clientB.Login(stealthscale.GetEndpoint(), pakClientB.Key)
	require.NoError(t, err)
...
func TestHASubnetRouterPingFailover(t *testing.T) {
	...
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		pr, err := stealthscale.PrimaryRoutes()
		assert.NoError(c, err)
	...
	// Block NEW outbound TCP from router 1 to stealthscale.
	// Preserves the existing Noise HTTP/2 long-poll (ESTABLISHED).
	hsIP := stealthscale.GetIPInNetwork(usernet1)
	iptablesAdd := []string{
		"iptables", "-A", "OUTPUT",
		"-d", hsIP,
		"-p", "tcp", "--dport", "8080",
	...
	t.Logf("Blocked new TCP connections from %s to stealthscale at %s:8080",
...
// See https://github.com/tomiwebpro/stealthscale/issues/3203.
//
...
// stealthscale via iptables and then removes the block to mimic the
...
func TestHASubnetRouterFailoverBothOfflineCablePull(t *testing.T) {
	...
	// "Cable pull" — drop all traffic in BOTH directions to/from stealthscale.
...
func TestHASubnetRouterFailoverDockerDisconnect(t *testing.T) {
	...
	// requirePrimary blocks until stealthscale reports want as the
