...
import (
	...
	"github.com/tomiwebpro/stealthscale/integration/tsric"
)

// TestTailscaleRustAxum tests that the tailscale-rs axum example can join a
// stealthscale network and serve HTTP to other peers on the tailnet.
//
// Architecture:
//
//	stealthscale (control) <--- tsic (probe client) --curl--> tsric (axum server)
//
// The test:
//  1. Creates a stealthscale environment with one regular Tailscale client (tsic)
//  2. Creates a tailscale-rs container running the axum example (tsric)
//  3. Verifies the tsric node registers with stealthscale
...
func TestTailscaleRustAxum(t *testing.T) {
	...
	err = scenario.CreateStealthScaleEnv(
		...
		// stealthscale CA into its trust chain; until then the control
		// plane has to be plain HTTP for the Rust client to register.
		hsic.WithoutTLS(),
	)
	requireNoErrStealthScaleEnv(t, err)

	// Get the stealthscale instance and probe client
	...
	pak, err := stealthscale.CreateAuthKey(userID, false, true)
	require.NoError(t, err)

	// Determine the network and stealthscale connection details
	networks := scenario.Networks()
	require.NotEmpty(t, networks)

	network := networks[0]
	stealthscaleIP := stealthscale.GetIPInNetwork(network)
	stealthscaleHostname := stealthscale.GetHostname()
	stealthscaleEndpoint := stealthscale.GetEndpoint()

	t.Logf("StealthScale endpoint: %s (hostname: %s, IP: %s)",
		stealthscaleEndpoint, stealthscaleHostname, stealthscaleIP)

	// Create the tailscale-rs container
	tsrsOpts := []tsric.Option{
		tsric.WithNetwork(network),
		tsric.WithStealthScaleURL(stealthscaleEndpoint),
		tsric.WithAuthKey(pak.Key),
		tsric.WithExtraHosts([]string{stealthscaleHostname + ":" + stealthscaleIP}),
	}
	...
	// Wait for the tailscale-rs node to appear in stealthscale's node list.
	...
	t.Log("Waiting for tailscale-rs node to register with stealthscale...")
	...
	}, 120*time.Second, 2*time.Second, "tailscale-rs node should register with stealthscale")
	...
	// so we can't curl via IPv6, but stealthscale should still assign both.
	assert.NotEmpty(t, rustNodeIPv6,
		"stealthscale should assign both IPv4 and IPv6 to the tailscale-rs node")
