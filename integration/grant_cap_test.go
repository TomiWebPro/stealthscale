...
func TestGrantCapRelay(t *testing.T) {
	...
	require.NoError(t, err)
	err = relayR.Login(stealthscale.GetEndpoint(), pakRelay.Key)
	require.NoError(t, err)
...
func TestGrantCapDrive(t *testing.T) {
	...
	require.NoError(t, err)
	err = sharer.Login(stealthscale.GetEndpoint(), pakSharer.Key)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	err = rwClient.Login(stealthscale.GetEndpoint(), pakRW.Key)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	err = roClient.Login(stealthscale.GetEndpoint(), pakRO.Key)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	err = noAccess.Login(stealthscale.GetEndpoint(), pakNA.Key)
	require.NoError(t, err)
