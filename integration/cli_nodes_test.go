...
func TestNodeCommand(t *testing.T) {
	...
	err = scenario.CreateStealthScaleEnv([]tsic.Option{}, hsic.WithTestName("cli-node"))
	require.NoError(t, err)
...
func TestNodeExpireCommand(t *testing.T) {
	...
	err = scenario.CreateStealthScaleEnv([]tsic.Option{}, hsic.WithTestName("cli-nodeexpire"))
	require.NoError(t, err)
...
func TestNodeRenameCommand(t *testing.T) {
	...
	err = scenario.CreateStealthScaleEnv([]tsic.Option{}, hsic.WithTestName("cli-noderename"))
	require.NoError(t, err)
...
func TestPreAuthKeyCorrectUserLoggedInCommand(t *testing.T) {
	...
	u2, err := stealthscale.CreateUser(user2)
	require.NoError(t, err)
...
func TestNodeExpireFlagsCommand(t *testing.T) {
	...
	scenario, stealthscale := setupCLIScenario(t, "cli-nodeexpireflags", []string{"expire-flags-user"}, 0)
	defer scenario.ShutdownAssertNoPanics(t)

	regID := types.MustAuthID().String()

	_, err := stealthscale.Execute([]string{
		"stealthscale", "debug", "create-node",
		"--name", "flagnode",
	...
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		err := executeAndUnmarshal(stealthscale,
			[]string{
				"stealthscale", "auth", "register",
				"--user", "expire-flags-user",
				"--auth-id", regID,
				"--output", "json",
			},
	...
	listNodeByID := func(ct *assert.CollectT) *clientv1.Node {
		...
		err := executeAndUnmarshal(stealthscale,
			[]string{"stealthscale", "nodes", "list", "--output", "json"},
			&nodes,
		)
...
func TestNodeCommandValidation(t *testing.T) {
	...
	scenario, stealthscale := setupCLIScenario(t, "cli-nodeval", []string{"user1"}, 0)
	defer scenario.ShutdownAssertNoPanics(t)
	...
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		err := executeAndUnmarshal(stealthscale,
			[]string{"stealthscale", "auth", "register", "--user", "user1", "--auth-id", regID, "--output", "json"},
			&node,
		)
		assert.NoError(c, err)
	}, integrationutil.ScaledTimeout(10*time.Second), integrationutil.FastPoll, "Waiting for node registration")
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := stealthscale.Execute(append([]string{"stealthscale"}, tt.args...))
			if tt.wantErr == "" {
				require.Error(t, err)
...
// TestNodeTagCommand exercises `stealthscale nodes tag` against a live node: it
...
func TestNodeTagCommand(t *testing.T) {
	...
	tagged := assertJSONRoundtrip[*clientv1.Node](t, stealthscale, []string{
		"stealthscale", "nodes", "tag",
		"--identifier", idStr,
...
func TestNodeRouteCommands(t *testing.T) {
	...
	assert.EventuallyWithT(t, func(ct *assert.CollectT) {
		...
		err := executeAndUnmarshal(stealthscale,
			[]string{"stealthscale", "nodes", "list-routes", "--output", "json"},
			&nodes,
		)
		assert.NoError(ct, err)
	...
	approved := assertJSONRoundtrip[*clientv1.Node](t, stealthscale, []string{
		"stealthscale", "nodes", "approve-routes",
		"--identifier", idStr,
		"--routes=" + route,
		"--output", "json",
	})
	...
	assert.EventuallyWithT(t, func(ct *assert.CollectT) {
		...
		err := executeAndUnmarshal(stealthscale,
			[]string{"stealthscale", "nodes", "list-routes", "--identifier", idStr, "--output", "json"},
			&nodes,
		)
		assert.NoError(ct, err)
		assert.Len(ct, nodes, 1)
	...
	cleared := assertJSONRoundtrip[*clientv1.Node](t, stealthscale, []string{
		"stealthscale", "nodes", "approve-routes",
		"--identifier", idStr,
		"--routes=",
		"--output", "json",
...
func TestNodeBackfillIPsCommand(t *testing.T) {
	...
	scenario, stealthscale := setupCLIScenario(t, "cli-backfillips", []string{"user1"}, 2)
	defer scenario.ShutdownAssertNoPanics(t)
	...
	assert.EventuallyWithT(t, func(ct *assert.CollectT) {
		...
		before, err = stealthscale.ListNodes()
		assert.NoError(ct, err)
	...
	out, err := stealthscale.Execute([]string{"stealthscale", "nodes", "backfillips", "--force"})
	require.NoError(t, err)
	assert.Contains(t, out, "backfilled")

	// Nodes must still have their IP addresses afterwards.
	assert.EventuallyWithT(t, func(ct *assert.CollectT) {
		after, err := stealthscale.ListNodes()
		assert.NoError(ct, err)
