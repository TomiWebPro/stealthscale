...
// Reported: https://github.com/tomiwebpro/stealthscale/issues/699
...
// https://github.com/tomiwebpro/stealthscale/issues/941
//
//	ACL = [{
//			"DstPorts": [{
//				"Bits": null,
...
// https://github.com/tomiwebpro/stealthscale/issues/809
func TestACLNamedHostsCanReach(t *testing.T) {
	IntegrationSkip(t)

	tests := map[string]struct {
		policy policyv2.Policy
		// Addresses covered by the policy. Address-based aliases (Prefix,
		// Host) resolve to exactly the literal prefix, so we can only curl
		// the address family defined in the policy's Hosts map.
		test1Addr string
		test2Addr string
		test3Addr string
	}{
	...
	for name, testCase := range tests {
		t.Run(name, func(t *testing.T) {
			...
			// Since user/users don't matter here, we basically expect that
			// some clients will be assigned these ips and that we can pick
			// them up for our own use. Look up clients by their IPv6
			// address (always assigned) regardless of test variant.
			...
			_, err = test3.CurlFailFast(test2URL)
			require.Error(t, err)
...
func TestACLDevice1CanAccessDevice2(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"group": {
			// Identity-based aliases resolve to all of a node's IPs, so
			// either address family works. Use IPv4 for consistency.
			test1Addr: "100.64.0.1",
			test2Addr: "100.64.0.2",
			policy: policyv2.Policy{
				Groups: policyv2.Groups{
	...
	for name, testCase := range tests {
		t.Run(name, func(t *testing.T) {
			...
			// Build URLs from the addresses the policy actually covers.
			// Address-based aliases (Prefix, Host) only resolve to the
			// literal prefix, so we must curl the matching address family.
			test1URL := fmt.Sprintf("http://%s/etc/hostname", testCase.test1Addr)
			test2URL := fmt.Sprintf("http://%s/etc/hostname", testCase.test2Addr)
...
func TestACLAutogroupTagged(t *testing.T) {
	...
	// Create only the stealthscale server (not the full environment with users/nodes)
	stealthscale, err := scenario.StealthScale(
		hsic.WithACLPolicy(policy),
	...
	for _, userStr := range spec.Users {
		...
		for i := range spec.NodesPerUser {
			...
			// [tsic.WithCACert] and [tsic.WithStealthScaleName] are passed explicitly because
			// nodes created via [Scenario.CreateTailscaleNode] are not part of
			// the standard [Scenario.CreateStealthScaleEnv] flow.
			opts := []tsic.Option{
				tsic.WithCACert(stealthscale.GetCert()),
				tsic.WithStealthScaleName(stealthscale.GetHostname()),
			...
			err = tsClient.Login(stealthscale.GetEndpoint(), authKey)
			require.NoError(t, err)
...
func TestACLAutogroupSelf(t *testing.T) {
	...
	// [Scenario.CreateStealthScaleEnv] flow.
	...
	err = routerClient.Login(stealthscale.GetEndpoint(), authKey.Key)
	require.NoError(t, err)
...
func TestACLPolicyPropagationOverTime(t *testing.T) {
	...
	for i := range 5 {
		...
		err = stealthscale.SetPolicy(allowAllPolicy)
		require.NoError(t, err)
		...
		err = stealthscale.SetPolicy(autogroupSelfPolicy)
		require.NoError(t, err)
		...
		newClient := scenario.MustAddAndLoginClient(t, "user1", "all", stealthscale,
			tsic.WithNetfilter("off"),
			tsic.WithPackages("curl"),
			tsic.WithWebserver(80),
			tsic.WithDockerWorkdir("/"),
		...
		assert.EventuallyWithT(t, func(ct *assert.CollectT) {
			nodeList, err = stealthscale.ListNodes("user1")
			assert.NoError(ct, err)
		...
		// Delete the node via stealthscale helper
		t.Logf("Iteration %d: Phase 2b - Deleting node ID %d from stealthscale", iteration, nodeToDeleteID)
		err = stealthscale.DeleteNode(nodeToDeleteID)
		...
		assert.EventuallyWithT(t, func(ct *assert.CollectT) {
			nodeListAfter, err := stealthscale.ListNodes("user1")
		...
		err = stealthscale.SetPolicy(user1ToUser2Policy)
		require.NoError(t, err)
...
// to ACLs without requiring a StealthScale restart.
...
func TestACLTagPropagation(t *testing.T) {
	...
	tests := []struct {
		...
		setup func(t *testing.T, scenario *Scenario, stealthscale ControlServer) (
	...
	}{
		{
			...
			setup: func(t *testing.T, scenario *Scenario, stealthscale ControlServer) (TailscaleClient, TailscaleClient, uint64) {
				t.Helper()
				...
				nodes, err := stealthscale.ListNodes("user1")
				require.NoError(t, err)
		...
		},
		{
			...
			setup: func(t *testing.T, scenario *Scenario, stealthscale ControlServer) (TailscaleClient, TailscaleClient, uint64) {
				...
				userMap, err := stealthscale.MapUsers()
				require.NoError(t, err)
				...
				require.NoError(t, err)
				err = user1Node.Login(stealthscale.GetEndpoint(), taggedKey.Key)
				require.NoError(t, err)
				...
				require.NoError(t, err)
				err = user2Node.Login(stealthscale.GetEndpoint(), untaggedKey.Key)
				require.NoError(t, err)
				...
				allNodes, err := stealthscale.ListNodes()
				require.NoError(t, err)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			sourceClient, targetClient, targetNodeID := tt.setup(t, scenario, stealthscale)
			...
			err = stealthscale.SetNodeTags(targetNodeID, tt.tagChange)
			require.NoError(t, err)
...
func TestACLTagPropagationPortSpecific(t *testing.T) {
	...
	err = stealthscale.SetNodeTags(targetNodeID, []string{"tag:sshonly"})
	require.NoError(t, err)

	// Step 2b: Verify tag was actually applied on the server before
	// checking client-side effects. Without this, the client assertions
...
func TestACLGroupWithUnknownUser(t *testing.T) {
	...
	//
	// Issue: https://github.com/tomiwebpro/stealthscale/issues/2967
...
func TestACLGroupAfterUserDeletion(t *testing.T) {
	...
	// This test verifies that when a user is deleted from stealthscale but
	...
	t.Log("Step 2: Deleting user3's node and user from stealthscale")

	// First, get user3's node ID
	nodes, err := stealthscale.ListNodes("user3")
	require.NoError(t, err)
	...
	_, err = stealthscale.CreateUser("user4")
	require.NoError(t, err, "failed to create user4")

	// Verify user4 was created
	_, err = GetUserByName(stealthscale, "user4")
...
func TestACLGroupDeletionExactReproduction(t *testing.T) {
	...
	nodes, err := stealthscale.ListNodes(userToDelete)
	require.NoError(t, err)
	require.Len(t, nodes, 1)
	err = stealthscale.DeleteNode(mustParseID(nodes[0].Id))
	require.NoError(t, err)

	userToDeleteObj, err := GetUserByName(stealthscale, userToDelete)
	require.NoError(t, err, "user to delete should exist")

	err = stealthscale.DeleteUser(mustParseID(userToDeleteObj.Id))
	require.NoError(t, err)
	...
	filter, err := stealthscale.DebugFilter()
	require.NoError(t, err)
...
func TestACLDynamicUnknownUserAddition(t *testing.T) {
	...
	err = stealthscale.SetPolicy(policyWithUnknown)
	require.NoError(t, err)
...
func TestACLDynamicUnknownUserRemoval(t *testing.T) {
	...
	err = stealthscale.SetPolicy(cleanPolicy)
	require.NoError(t, err)
