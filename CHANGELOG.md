...
## 0.0.1-alpha.4 (2026-09-02) — Docker compatibility (full container support)

Docker was the last blocked platform (`kos` disabled since `alpha.1`, `alpha.2`/`alpha.3` were native-only: `No Docker images in this channel`). With Windows tray + clean uninstall proven, `ghcr.io/tomiwebpro/stealthscale` is re-enabled and the entire stack is verified for container deployment — **targeting full compatibility of this program with Docker**.

- **Container images (kos re-enabled, `alpha.4`)**: `.goreleaser.yml:15` comment `Next alpha: all OS/arch except Docker` → `Alpha 4: Docker enabled via kos`, `kos:` uncommented and hardened (`ghcr` + `ghcr-debug`, `gcr.io/distroless/base-debian13` + `:debug`, `linux/amd64,arm64`, tags `latest`/`stable`/`unstable`/`v0.0.1-alpha.4`/`sha-*`, `bare:true`, `CGO_ENABLED=0`). Description `nfpms:76` now mentions VLESS+Reality + Docker `ghcr.io/tomiwebpro/stealthscale`. `goreleaser check` still passes with 11 native targets (`linux_amd64/arm64/arm_6/arm_7`, `darwin`, `windows`, `freebsd`) plus `kos`.
- **Production Dockerfile (`Dockerfile:1`)**: standalone `debian:trixie-slim` image (`golang:1.26.5-trixie` builder → `stscale` at `/usr/local/bin/stscale` with `stealthscale` symlink for legacy CLI), `stealthscale:stealthscale` user, `VOLUME ["/var/lib/stealthscale","/etc/stealthscale"]` for `db.sqlite`/`.xray_secret` persistence, `EXPOSE 8080/tcp 9090/tcp 3478/udp 10001-10100/tcp`, `HEALTHCHECK CMD ["stscale","health"]`. Ko remains primary (`kos` → `gcr.io/distroless/base-debian13` at `/ko-app/stscale`), `Dockerfile` is for `docker build -f Dockerfile --build-arg VERSION=...` and local dev. `Dockerfile.integration:33` + `Dockerfile.integration-ci:13` updated with same `VOLUME`/`EXPOSE`/`HEALTHCHECK` and `stealthscale` symlink (`ln -sf stscale → stealthscale`) so legacy `integration/*.go` `Execute(["stealthscale",...])` keeps working while new code uses `stscale`.
- **Integration harness (`hscontrol`/`integration` docker)**: `integration/hsic/hsic.go:54` `binStealthScale="stealthscale"` → `"stscale"` (canonical binary), `buildEntrypoint:321` `"/usr/local/bin/stealthscale serve"` → `"/usr/local/bin/stscale serve"` (symlink keeps compat), postgres env `STEALTHSCALE_DATABASE_POSTGRES_USER/PASS/NAME` now hard-coded `"stealthscale"` (was `binStealthScale` → would change DB name). `CreateOAuthClient:1039` now uses `binStealthScale` not literal. Integration `Dockerfile`s gain `mkdir -p /var/lib/stealthscale /etc/stealthscale` before `COPY`. `xray.secret` persistence is volume-gated: `hscontrol/types/config.go:367` `loadOrCreateSecret` + `validateServerConfig:989` `xray.secret required for postgres` ensures Docker `postgres` deployments cannot silently lose identity (set `STEALTHSCALE_XRAY_SECRET=$(openssl rand -hex 32)` explicitly, docs updated).
- **CI (` .github/workflows/ci.yml:28,97,130`)**: `test` job gains `Check docker compatibility (alpha.4)` (verifies `^kos:` enabled, `ghcr`, `distroless`, `Dockerfile` existence/`HEALTHCHECK`/`VOLUME`/`10001`, `hsic.go` `binStealthScale="stscale"` and `stscale serve` entrypoint, plus `go test -run TestDocker ./hscontrol`). `cross-build` `Check goreleaser multi-platform` now also checks `^kos:` + `ghcr` + `linux/amd64,arm64` + `Dockerfile`. New job `docker-compat` builds both images (`docker build -f Dockerfile` + `Dockerfile.integration`) and smoke-tests a running container (`docker run -p 18080:8080`, `curl /health`, `stscale health`, `ls /var/lib/stealthscale`). `goreleaser-check` gains `goreleaser check --snapshot`. `path-tests` unchanged.
- **Tests (docker, `alpha.4`)**: `hscontrol/docker_compat_test.go:12` 13 tests: `TestDockerGoreleaserKosEnabled`, `BuildsAllPlatforms`, `DockerfileProduction/Integration/CI`, `ConfigEnvPrefix`, `XRaySecretRequiredForPostgres`, `VolumesAndStateDir`, `UnixSocket`, `HealthEndpoint`, `ComposeExample`, `LabelsAndEnv`, `VLESSDefaults`. `integration/docker_test.go:19` 3 integration tests (`TestDockerHealthAndBasicServe`, `VLESSPersistence`, `PortBindingsAndVolumes`) that drive real Docker containers via `hsic` (`IntegrationSkip`), verify `stscale health` + `stealthscale` symlink, `pidof stscale`, `ss -tlnp` VLESS `10001`, `.xray_secret` 64-hex stability across `Restart()`, `9090/metrics`, `3478/udp`, `/var/lib` writability and `stscale version`.
- **Docs/packaging**: `packaging/README.md:3` no longer `no Docker until…`, new `Docker (distroless via kos + standalone Dockerfile)` section with `ghcr.io/tomiwebpro/stealthscale:<VERSION>` tags and volume/persist guidance. `docs/setup/install/container.md:7` `kos disabled` warning removed → `alpha.4 enables full Docker compatibility via kos` + platforms/tags/legacy/manual build. `docs/setup/install/main.md:7` same. `docs/about/versioning.md` unchanged (alpha is `isDev=false`, writes `database_versions` as `0.0.1`). Published images: `ghcr.io/tomiwebpro/stealthscale:v0.0.1-alpha.4` (also `unstable`, `sha-<commit>`, `...-debug`, `latest-debug` not for prerelease) for `linux/amd64,arm64` (`docker pull` + `ls /ko-app` for debug).

No breaking changes — `isDev` still gates `database_versions` poisoning (`nightly`/`-next`/`dirty` skip, `alpha.*`/`beta`/`rc`/`stable` write core `0.0.1`). Upgrade from `alpha.3` is `minorDiff==0` → allowed. Run `docker pull ghcr.io/tomiwebpro/stealthscale:v0.0.1-alpha.4` or `ghcr.io/tomiwebpro/stealthscale:unstable` / `sha-...`; `:debug` has `busybox` for `curl`/`ls /ko-app`.

## 0.0.1-alpha.3 (2026-09-02) — Windows usability (hide-in-tray) + clean uninstall (all OS)

Windows crash (“no UI, no tray, binary crashes”) is fixed and native usability promoted to first-class. `dist/` `stscale.exe` is now correctly cross-built from the fixed `332185d` tree (previous `dist/` was stale Aug-31, pre-fix).

- **Windows tray (hide-in-tray, closes alpha.2 gap)**: `stscale serve --tray` via `fyne.io/systray` (`hscontrol/tray/tray_windows.go`, `golang.org/x/sys/windows` only, no CGO, `CGO_ENABLED=0` in `.goreleaser.yml:28`). Menu: Open WebUI (`http://127.0.0.1:8080/web` from `cfg.Addr`, `hscontrol/tray/weburl.go`), Status (polls `/health` every 5s → `checking…/online/offline`, `tray_windows.go:112`), Quit (triggers `os.Interrupt`+`SIGTERM` → `hscontrol/app.go:955` graceful). Icon `hscontrol/assets/tray.ico` (ICO 256/48/32/16 via `assets.go:TrayIcon`, fallback `favicon.png`), tooltip `StealthScale <version>`, close minimizes to tray. `packaging/windows/install.ps1 -LaunchAtStartup` now writes `HKCU\...\Run` → `serve --tray` (was placeholder, `install.ps1:104`, `packaging/windows/README.md:38`). `cmd/tray/README.md` updated from placeholder to implemented.
- **Clean uninstall (all distributions)**: `stscale uninstall [--purge] [--yes]` (`cmd/stealthscale/cli/uninstall.go`) + per-OS scripts: `packaging/windows/uninstall.ps1 -Purge`, `packaging/launchd/uninstall.sh --purge`, `packaging/systemd/uninstall.sh --purge`. Linux: `systemctl stop/disable`, remove `/usr/bin/stscale`, `--purge` deletes `/etc/stealthscale`, `/var/lib/stealthscale`, user (`packaging/deb/postrm:27`), `deb-systemd-helper purge`; `apt purge stealthscale` still works. Windows: `sc.exe stop/delete StealthScale`, `reg delete HKCU\...\Run\StealthScale`, `Remove-Item %ProgramFiles%\stealthscale`, `--purge` also `%ProgramData%\stealthscale` (config, `db.sqlite`, `.xray_secret`). macOS: `launchctl unload /Library/LaunchDaemons/com.stealthscale.plist`, remove `/usr/local/bin/stscale`, `/usr/local/etc/stealthscale`, `/Library/Application Support/stealthscale`, `/Library/Logs/stealthscale`; `--purge` deletes those. See `packaging/README.md:39` and `docs/stealthscale/install.md:131 Uninstall` (also `docs/setup/install/*`).
- **CLI/paths polish (windows crash leftovers, `gap-multios`)**: `cmd/stealthscale/cli/up.go:36` state-dir help now per-OS (`%ProgramData%`) and `loadOrCreateMachineKey:225` falls back to `os.TempDir()` not `/tmp` on Windows; `dump_config.go:19` hardened for `ProgramData` vs `/etc` (`EnsureDir`); `root.go:26` `--config` help `runtime.GOOS` switch (`C:\ProgramData\stealthscale\config.yaml` / `/usr/local/etc/stealthscale/config.yaml`); `hscontrol/app.go:1151 GetConfig()`, `assets.go:TrayIcon`, `tray/weburl.go` refactor.
- **WebUI hardening + view fixes (audit-20260901)**: WebUI `hscontrol/webui/webui.go:28` now validates `Authorization: Bearer <api-key>` / `X-API-Key` via `ValidateAPIKey`/`AuthenticateAccessToken` (not just presence), `derpMap`/`health` use `DERPMapView.MarshalJSON` not `AsStruct()` clone, `handleCreateUser/PreAuthKey` delegate correctly to `state.State` `change.Change` signatures — respects `AGENTS.md` view rule.
- **Tests (tray windows)**: `hscontrol/tray/tray_test.go:15` `TestBuildWebURL` (7 cases 0.0.0.0/::/https/invalid), `TestIsSupported` per-GOOS, `TestTrayIconEmbedded` ICO header `00 00 01 00`, `TestNeedsTray`; `cmd/stealthscale/cli/serve_tray_test.go:9` `TestServeTrayFlagRegistered`, `IsSupported`, `HasTrayHelp`; `uninstall_test.go:11` `TestUninstallCommandRegistered`, `HelpPerOS`. `GOOS=windows go vet` / `go build` passes (76M exe, `Subsystem 0x03 CUI` with tray), `CI .github/workflows/ci.yml:44` cross-vet matrix still green.
- **CI/packaging**: `dist/` rebuilt (was stale Aug-31); `goreleaser.yml:28` still `CGO_ENABLED=0` (systray Windows needs no CGO); `packaging/launchd`/`systemd` uninstall scripts added; `CHANGELOG` 0.0.1-alpha.2 remains.

No Docker images in this channel — native binaries only. `v0.0.1-alpha.3` is prerelease (`isDev=false`). Next promotion will re-enable `kos` (`ghcr.io/tomiwebpro/stealthscale`) once the tray + uninstall are proven on Windows/macOS.
