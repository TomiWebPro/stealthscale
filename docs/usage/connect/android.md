...
This documentation has the goal of showing how a user can use the official Android [Tailscale](https://tailscale.com) client with stscale.
...
- Enter your server URL (e.g `https://stscale.example.com`) and follow the instructions
- The client connects automatically as soon as the node registration is complete on stscale. Until then, nothing is
...
- Tap on `Accounts`
- In the kebab menu icon (three dots) in the upper-right corner select `Use an alternate server`
- Enter your server URL (e.g `https://stscale.example.com`). If login prompts open, close it and continue
...
- Enter your [preauthkey generated from stscale](../../ref/registration.md#pre-authenticated-key)
- If needed, tap `Log in` on the main screen. You should now be connected to your stscale.
