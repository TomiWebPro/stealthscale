...
This documentation has the goal of showing how a user can use the official Windows [Tailscale](https://tailscale.com) client with stscale.
...
Open a Command Prompt or Powershell and use Tailscale's login command to connect with your stscale instance (e.g
`https://stscale.example.com`):
...
in your stscale output, turn on `DEBUG` logging and look for:
...
1. Ensure the Windows node is deleted from stscale (to ensure fresh setup)
