...
StealthScale supports route advertising and can be used to manage [subnet
...
```console
$ sudo tailscale up --login-server <YOUR_STSCALE_URL> --advertise-routes=10.0.0.0/8,192.168.0.0/24
```
...
The routes of a tailnet can be displayed with the `stscale nodes list-routes` command. A subnet router with the
hostname `myrouter` announced the IPv4 networks `10.0.0.0/8` and `192.168.0.0/24`. Those need to be approved before they
can be used.

```console
$ stscale nodes list-routes
...
```console
$ stscale nodes approve-routes --identifier 1 --routes 10.0.0.0/8,192.168.0.0/24
Node updated
```
...
before they can be used by a node in a tailnet. StealthScale supports the `autoApprovers` section in a policy to automate
...
```console
$ sudo tailscale up --login-server <YOUR_STSCALE_URL> --advertise-tags tag:router --advertise-routes 192.168.0.0/24
```
...
```console
$ sudo tailscale up --login-server <YOUR_STSCALE_URL> --advertise-exit-node
```
...
The routes of a tailnet can be displayed with the `stscale nodes list-routes` command. An exit node can be recognized
...
```console
$ stscale nodes approve-routes --identifier 1 --routes 0.0.0.0/0
Node updated
```
...
in a tailnet. StealthScale supports the `autoApprovers` section in a policy to automate the approval of a new exit node as
...
```console
$ sudo tailscale up --login-server <YOUR_STSCALE_URL> --advertise-tags tag:exit --advertise-exit-node
```
...
StealthScale supports high availability routing. Multiple subnet routers with overlapping routes or multiple exit nodes can
