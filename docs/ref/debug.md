...
StealthScale and Tailscale provide debug and introspection capabilities that can be helpful when things don't work as
...
- Test DERP connection: `tailscale debug derp stscale`
- And many more, see: `tailscale debug --help`

Many of the commands are helpful when trying to understand differences between StealthScale and Tailscale SaaS.

## StealthScale

### Application logging

The log levels `debug` and `trace` can be useful to get more information from StealthScale.
...
The database debug mode logs all database queries. Enable it to see how StealthScale interacts with its database. This also
...
StealthScale provides a metrics and debug endpoint. It allows to introspect different aspects such as:
...
=== "Direct access"
    ...
    Access the debug interface directly on the server where StealthScale is installed.
...
=== "SSH port forwarding"
    ...
    Use SSH port forwarding to forward StealthScale's metrics and debug port to your device.

    ```console
    ssh <STSCALE_SERVER> -L 9090:localhost:9090
    ```
...
=== "Via debug key"
    ...
    Access the debug interface on your device by opening `http://<IP_OF_STSCALE>:9090/debug/?debugkey=<DEBUG_KEY>` in
...
=== "Via debug IP address"
    ...
    `TS_ALLOW_DEBUG_IP` environment variable before starting StealthScale. The debug IP address is ignored when the HTTP
    ...
    Access the debug interface on your device by opening `http://<IP_OF_STSCALE>:9090/debug/` in your web browser.
