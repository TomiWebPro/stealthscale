# Running StealthScale behind a reverse proxy

!!! warning "Community documentation"

    This page is not actively maintained by the StealthScale authors and is
    written by community members. It is _not_ verified by StealthScale developers.

    **It might be outdated and it might miss necessary steps**.

Running StealthScale behind a reverse proxy is useful when running multiple applications on the same server, and you want
...
The configuration depends on the set of StealthScale features you intend to use. Please have a look at the
...
STUN for StealthScale's [embedded DERP server](../derp.md) are expected to be exposed directly or to be only available on
localhost.

### WebSocket

Tailscale clients are using a custom protocol (Tailscale Control Protocol) to communicate with a control server such as
StealthScale. The reverse proxy **must** be configured to support WebSockets in order to communicate with Tailscale clients
...
StealthScale can be configured not to use TLS, leaving it to the reverse proxy to handle. Add the following configuration
values to your StealthScale [configuration file](../configuration.md):
...
StealthScale logs `WRN listening without TLS but ServerURL does not start with http://` during startup. This is expected
and indicates that the reverse proxy is in charge of terminating TLS.

### Trusted proxies

StealthScale ignores `True-Client-IP`, `X-Real-IP` and `X-Forwarded-For` headers unless the request's TCP peer matches the
...
on inbound requests with sanitized values. StealthScale picks the first valid IP address supplied by headers in this order:
...
This following StealthScale configuration may be used as base for the various reverse proxy examples below. The following
...
- Both StealthScale and the reverse proxy are running on the same host.
...
The following basic Apache configuration works with the StealthScale configuration [as shown
above](#reverse-proxy-specific-configuration). Substitute placeholders and adjust the configuration as needed:

- `<SERVER_NAME>`: The server name for your instance, e.g. `stscale.example.com`
...
The following basic Caddyfile works with the StealthScale configuration [as shown
above](#reverse-proxy-specific-configuration). Substitute placeholders and adjust the configuration as needed:
...
Running StealthScale behind a Cloudflare Proxy or Cloudflare Tunnel is not supported and will not work as Cloudflare does
not support [WebSocket POSTs as required by the Tailscale protocol](#websocket). See [issue
1468](https://github.com/tomiwebpro/stealthscale/issues/1468) for more information.
...
kind: EnvoyFilter
metadata:
  name: stscale-behind-istio-ingress
...
The following basic Nginx configuration works with the StealthScale configuration [as shown
above](#reverse-proxy-specific-configuration). Substitute placeholders and adjust the configuration as needed:
