...
StealthScale supports authentication via external identity providers using OpenID Connect (OIDC). It features:
...
OpenID requires configuration in StealthScale and your identity provider:

- StealthScale: The `oidc` section of the StealthScale [configuration](configuration.md) contains all available configuration
...
A basic configuration connects StealthScale to an identity provider and typically requires:

- OpenID Connect Issuer URL from the identity provider. StealthScale uses the OpenID Connect Discovery Protocol 1.0 to
  automatically obtain OpenID configuration parameters (example: `https://sso.example.com`).
- Client ID from the identity provider (example: `stscale`).
- Client secret generated by the identity provider (example: `generated-secret`).
- Redirect URI for your identity provider (example: `https://stscale.example.com/oidc/callback`).
...
=== "Identity provider"
    ...
    - Add StealthScale's OIDC callback URL as valid redirect URL: `https://stscale.example.com/oidc/callback`
...
recommended and needs to be configured for StealthScale and the identity provider alike:
...
=== "Identity provider"
    ...
    - Enable PKCE for the stscale client
    - Set the PKCE challenge method to "S256"

### Authorize users with filters

StealthScale allows to filter for allowed users based on their domain, email address or group membership. These filters can
...
=== "Allowed groups"
    ...
    - Access allowed: users in the `stealthscale_users` group
...
StealthScale uses the `email` claim from the identity provider to synchronize the email address to its user profile. By
...
!!! tip "Expire a node and force re-authentication"
    ...
    ```console
    stscale node expire -i <NODE_ID>
    ```

### Reference a user in the policy

You may refer to users in the StealthScale policy via:
...
!!! note "A user identifier in the policy must contain a single `@`"
    ...
    The StealthScale policy requires a single `@` to reference a user. If the username or provider identifier doesn't
    already contain a single `@`, it needs to be appended at the end. For example: the StealthScale username `ssmith` has
    to be written as `ssmith@` to be correctly identified as user within the policy.

    Ensure that the StealthScale username itself does not end with `@`.
...
!!! warning "Email address or username might be updated by users"
    ...
    consequences for StealthScale where a policy might no longer work or a user might obtain more access by hijacking an
...
StealthScale uses [the standard OIDC claims](https://openid.net/specs/openid-connect-core-1_0.html#StandardClaims) to
populate and update its local user profile on each login. OIDC claims are read from the ID Token and from the UserInfo
endpoint.

| StealthScale profile | OIDC claim           | Notes / examples                                                                                  |
| -------------------- | -------------------- | ------------------------------------------------------------------------------------------------- |
...
Please see the [GitHub label "OIDC"](https://github.com/tomiwebpro/stealthscale/labels/OIDC) for OIDC related issues.
...
!!! warning "Third-party software and services"
    ...
    section](#configuration) for a description of StealthScale's OIDC related configuration settings.

Any identity provider with OpenID Connect support should "just work" with StealthScale. The following identity providers
are known to work:
...
Authelia is fully supported by StealthScale.

### Authentik

- Authentik is fully supported by StealthScale.
- [StealthScale does not support JSON Web Encryption](https://github.com/tomiwebpro/stealthscale/issues/2446). Leave the field
  `Encryption Key` in the providers section unset.
- See Authentik's [Integrate with StealthScale](https://integrations.goauthentik.io/networking/stscale/)
...
!!! warning "No username due to missing preferred_username claim"
    ...
    StealthScale will be blank/not set.

In order to integrate StealthScale with Google, you'll need to have a [Google Cloud
...
1. Under `Authorised redirect URIs`, add StealthScale's OIDC callback URL: `https://stscale.example.com/oidc/callback`
...
1. [Configure StealthScale following the "Basic configuration" steps](#basic-configuration). The issuer URL for Google
   OAuth is: `https://accounts.google.com`.

### Kanidm

- Kanidm is fully supported by StealthScale.
- Groups for the [allowed groups filter](#authorize-users-with-filters) need to be specified with their full SPN, for
  example: `stealthscale_users@sso.example.com`.
- Kanidm sends the full SPN (`alice@sso.example.com`) as `preferred_username` by default. StealthScale stores this value as
  ...
  instead](https://kanidm.github.io/kanidm/stable/integrations/oauth2.html#short-names):
    ...
    ```
    Once configured, the short username in StealthScale will be `alice` and can be referred to as `alice@` in the policy.

### Keycloak

Keycloak is fully supported by StealthScale.
...
- Configure the new client scope for your StealthScale client:
    - Edit the StealthScale client.
    - Search for the client scope `group`.
    - Add it with assigned type `Default`.
- [Configure the allowed groups in StealthScale](#authorize-users-with-filters). How groups need to be specified depends on
...
In order to integrate StealthScale with Microsoft Entra ID, you'll need to provision an App Registration with the correct
scopes and redirect URI.

[Configure StealthScale following the "Basic configuration" steps](#basic-configuration). The issuer URL for Microsoft
...
StealthScale OIDC scope without the `groups` claim, for example:
...
StealthScale only supports a single OIDC provider in its configuration, but it does store the provider identifier of each user. When switching providers, this might lead to issues with existing users: all user details (name, email, groups) might be identical with the new provider, but the identifier will differ. StealthScale will be unable to create a new user as the name and email will already be in use for the existing users.
