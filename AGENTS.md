...
# Individual targets
make build           # build the stealthscale binary
make test            # go test ./...
...
.stealthscale/
├── cmd/
│   ├── stealthscale/  # Main stealthscale server binary
...
StealthScale enforces **tags XOR user ownership**: every node is either
...
- External calls (`client.Status`, `stealthscale.ListNodes`, etc.) belong
