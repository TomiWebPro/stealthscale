...
func TestAuthenticationFlows(t *testing.T) {
	...
	tests := []struct {
		name        string
		setupFunc   func(*testing.T, *StealthScale) (string, error) // Returns dynamic values like auth keys
		...
		wantAuthURL bool
		wantExpired bool
		validate    func(*testing.T, *tailcfg.RegisterResponse, *StealthScale)
		...
		// WHY: Pre-auth keys enable automated/headless node registration without user interaction
		{
			...
			setupFunc: func(t *testing.T, app *StealthScale) (string, error) { //nolint:thelper //nolint:thelper // not a test helper, inline closure
			...
			wantAuth:   true,
			validate: func(t *testing.T, resp *tailcfg.RegisterResponse, app *StealthScale) { //nolint:thelper //nolint:thelper // not a test helper, inline closure
		...
		// WHY: Reusable keys allow multiple machines to join using one key (useful for fleet deployments)
		{
			...
			setupFunc: func(t *testing.T, app *StealthScale) (string, error) { //nolint:thelper //nolint:thelper
			...
			wantAuth:   true,
			validate: func(t *testing.T, resp *tailcfg.RegisterResponse, app *StealthScale) { //nolint:thelper //nolint:thelper
				assert.True(t, resp.MachineAuthorized)
				assert.False(t, resp.NodeKeyExpired)
		...
		// WHY: Followup mechanism allows nodes to poll/wait for auth completion
		{
			...
			setupFunc: func(t *testing.T, app *StealthScale) (string, error) { //nolint:thelper //nolint:thelper
				...
				// [StealthScale.handleRegister] will receive the value when it starts waiting
				go func() {
		...
		// WHY: Pre-auth keys can enforce ACL policies on nodes during registration
		{
			...
			machineKey: machineKey1.Public,
			wantAuth:   true,
			validate: func(t *testing.T, resp *tailcfg.RegisterResponse, app *StealthScale) { //nolint:thelper
				assert.True(t, resp.MachineAuthorized)
				assert.False(t, resp.NodeKeyExpired)
		...
		// WHY: PreAuthKey nodes get their tags from the key itself, not from client requests
		{
			...
			setupFunc: func(t *testing.T, app *StealthScale) (string, error) { //nolint:thelper
				t.Helper()
...
func runInteractiveWorkflowTest(t *testing.T, tt struct {
	name                      string
	setupFunc                 func(*testing.T, *StealthScale) (string, error)
	request                   func(dynamicValue string) tailcfg.RegisterRequest
	machineKey                func() key.MachinePublic
	wantAuth                  bool
	wantError                 bool
	...
	simulateAuthCompletion    bool
	validateCompleteResponse  bool
}, app *StealthScale, dynamicValue string,
) {
	t.Helper()
...
// 3. Admin runs: stealthscale auth register --auth-id <id> --user user2
//
// Expected behavior:
...
func TestWebFlowReauthDifferentUser(t *testing.T) {
	...
	// This simulates: stealthscale auth register --auth-id <id> --user user2
...
func createTestApp(t *testing.T) *StealthScale {
	t.Helper()
	...
	cfg := types.Config{
		...
		Database: types.DatabaseConfig{
			...
			Sqlite: types.SqliteConfig{
				Path: tmpDir + "/stealthscale_test.db",
			},
		},
...
// https://github.com/tomiwebpro/stealthscale/issues/2830
//
// Scenario:
// 1. Node registers successfully with a single-use pre-auth key
// 2. Node is running fine
// 3. Node restarts (e.g., after stealthscale upgrade or tailscale container restart)
// 4. Node sends [tailcfg.RegisterRequest] with the same pre-auth key
// 5. BUG: StealthScale rejects the request with "authkey expired" or "authkey already used"
//
// Expected behavior:
...
//
// This is the fix for GitHub issue #2830: https://github.com/tomiwebpro/stealthscale/issues/2830
//
...
func TestTaggedNodeWithoutUserToDifferentUser(t *testing.T) {
	...
	// This is what happens when running: stealthscale auth register --auth-id <id> --user alice
	nodeKey2 := key.NewNode()
	registrationID := types.MustAuthID()
	regEntry := types.NewRegisterAuthRequest(&types.RegistrationData{
...
// [StealthScale.waitForFollowup]. That poll is authenticated only by the auth ID
...
//
// [StealthScale.handleRegister] and [StealthScale.handleLogout] already fail closed
...
// https://github.com/tomiwebpro/stealthscale/issues/3385.
//
// Root cause: [StealthScale.waitForFollowup] selects on ctx.Done() and the auth
