...
func TestSQLiteMigrationAndDataValidation(t *testing.T) {
	tests := []struct {
		...
		// at 14:15:06 ❯ go run ./cmd/stealthscale preauthkeys list
		...
		// Fixes: https://github.com/tomiwebpro/stealthscale/issues/3284
		{
		...
		// Fixes: https://github.com/tomiwebpro/stealthscale/issues/3323
		{
		...
		// Fixes: https://github.com/tomiwebpro/stealthscale/issues/3371
		{
...
func TestPostgresMigrationAndDataValidation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			db := newStealthScaleDBFromPostgresURL(t, u)
...
func dbForTestWithPath(t *testing.T, sqlFilePath string) *HSDatabase {
	...
	dbPath := t.TempDir() + "/stealthscale_test.db"
	...
	db, err := NewStealthScaleDatabase(
		&types.Config{
...
//
// All test database files are SQL dumps (created with `sqlite3 stealthscale.db .dump`) generated
// with old StealthScale binaries on empty databases (no user/node data). These dumps include the
