...
// DatabaseVersion tracks the stealthscale version that last
...
// checkVersionUpgradePath verifies that the running stealthscale version
...
func checkVersionUpgradePath(db *gorm.DB) error {
	...
	if isDev(currentVersion) {
		...
		if storedVersion != "" && !isDev(storedVersion) {
			log.Warn().
				...
				Msg("running a development build of stealthscale without a version number, " +
	...
	if current.Major != stored.Major {
		return fmt.Errorf(
			"stealthscale version %s cannot be used with a database last used by %s: %w",
	...
	case minorDiff > 1:
		...
		return fmt.Errorf(
			"stealthscale version %s cannot be used with a database last used by %s, "+
				...
				"release page: https://github.com/tomiwebpro/stealthscale/releases: %w",
