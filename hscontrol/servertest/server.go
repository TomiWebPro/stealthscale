// Package servertest provides an in-process test harness for StealthScale's
// control plane. It wires a real StealthScale server to real Tailscale
...
import (
	...
	hscontrol "github.com/tomiwebpro/stealthscale/hscontrol"
...
// WithXRay enables the VLESS transport. addr is the address the per-node
// listeners bind to (use "127.0.0.1" for real loopback sockets), and
// portStart/portEnd bound the deterministic per-node port range. The
// listeners are not started until [TestServer.StartXRay] is called.
func WithXRay(addr string, portStart, portEnd int) ServerOption {
...
func NewServer(tb testing.TB, opts ...ServerOption) *TestServer {
	...
	if sc.xrayAddr != "" {
		cfg.XRay = types.XRayConfig{
			Enabled:        true,
			ListenAddr:     sc.xrayAddr,
	...
	app, err := hscontrol.NewStealthScale(&cfg)
	if err != nil {
		tb.Fatalf("servertest: NewStealthScale: %v", err)
	}
...
// StartXRay boots the VLESS transport listeners for every node currently in
// the store. Nodes registered afterwards get their listener automatically on
// registration. Requires WithXRay to have been passed to NewServer. The
// listeners are torn down on test cleanup.
func (s *TestServer) StartXRay(tb testing.TB) {
	tb.Helper()

	if err := s.App.StartXRayServer(context.Background()); err != nil {
		tb.Fatalf("servertest: StartXRayServer: %v", err)
	}

	tb.Cleanup(func() {
		ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
		defer cancel()
		s.App.StopXRayServer(ctx)
	})
}

// Close shuts down the in-memory HTTP server and listener.
// Subsystem cleanup (batcher, ephemeral GC) is handled by
// [testing.TB.Cleanup] callbacks registered in [hscontrol.StealthScale.StartBatcherForTest] and
// [hscontrol.StealthScale.StartEphemeralGCForTest].
