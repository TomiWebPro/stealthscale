...
// Terraform provider (via OpenTofu). All three run against one StealthScale bound
...
func apiv2UsersGoClient(t *testing.T, srv *servertest.TestServer, baseURL, apiKey string, owner *types.User) {
	...
	// member matches every StealthScale user; shared/admin match nothing.
	...
	require.NoError(t, err)
	assert.Empty(t, shared, "StealthScale has no shared users")

	admins, err := ur.List(ctx, nil, new(tsclient.UserRoleAdmin))
	require.NoError(t, err)
	assert.Empty(t, admins, "StealthScale has no admin-role users")
...
// prove that data source resolves against StealthScale unchanged. %s is the owner's
