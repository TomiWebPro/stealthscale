...
type TestClient struct {
	...
	machineKey key.MachinePrivate // test-only access to the client identity
...
// MachineKey returns the client's machine private key, used by tests that
// speak the control protocol over a transport other than the default one
// (e.g. the VLESS path) while impersonating the same machine.
func (c *TestClient) MachineKey() key.MachinePrivate {
	return c.machineKey
}
