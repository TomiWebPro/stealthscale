...
func runViaHACompat(t *testing.T, c *testcapture.Capture) {
	...
	// Create nodes in SaaS node ID order so stealthscale assigns
...
// compareCaptureNetmap compares stealthscale's [tailcfg.MapResponse] against a
...
func compareCaptureNetmap(
	...
	// Build peer summaries from stealthscale [tailcfg.MapResponse].
	...
	for name, wantPeer := range wantPeers {
		...
		if !visible {
			t.Errorf("peer %s: visible in SaaS, missing in stealthscale (routes=%v)",
	...
	for name := range gotPeers {
		if _, expected := wantPeers[name]; !expected {
			t.Errorf("peer %s: visible in stealthscale but NOT in SaaS", name)
		}
	}
	...
	if len(want.PacketFilterRules) > 0 {
		...
		assert.Equalf(t, len(want.PacketFilterRules), gotLen,
			"PacketFilter rule count mismatch (SaaS=%d, stealthscale=%d)",
...
// convertCapturePolicy converts a [testcapture.Capture]'s policy for stealthscale,
// replacing SaaS emails with stealthscale user format. Fails the test if
