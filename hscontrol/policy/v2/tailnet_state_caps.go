...
// Tailscale-hosted control plane emits where stealthscale has no
...
// source), why stealthscale does not emit it, and a tracking issue where
// one exists.
...
//
//  1. Caps gated on a user-role concept stealthscale does not model.
//  2. Caps gated on a tailnet feature stealthscale does not implement.
//  3. Caps that are tailnet-state metadata (display name, key
//     duration, etc.) where the values are not derivable from
//     stealthscale config in a way that round-trips through the
//     anonymized capture.
//  4. Caps that are internal magicsock or embedded-SSH tuning with no
//     stealthscale-side equivalent.
var unmodelledTailnetStateCaps = []tailcfg.NodeCapability{
	...
	// inherit from a tagOwner with the role. StealthScale has no
	...
	// on the "owner" role rather than admin. StealthScale does not emit
	...
	// ipn/ipnlocal/local.go:1752 (b.capTailnetLock). StealthScale has no
	...
	// StealthScale does not implement Tailscale Services.
	...
	// controlknobs:148. StealthScale does not implement app connectors.
	...
	// --- 3. Tailnet-state metadata not derivable from stealthscale config ---

	// [tailcfg.NodeAttrTailnetDisplayName]: tailnet display name
	// surfaced in the client UI. The hosted control plane emits the
	// tailnet admin's email; stealthscale would have to invent a value
	...
	// value. StealthScale has cfg.Node.Expiry but does not surface it
	...
	// --- 4. Internal tuning, no stealthscale equivalent ---
