...
// Tailscale-hosted control plane, and compares stealthscale's
...
// TODO comment explaining the root cause. As stealthscale's nodeAttrs
...
// stealthscale deliberately rejects at validate time. The rejection itself is
...
//	    user-role and tailnet-ownership concepts stealthscale does not
//	    model.
...
// nodeAttrs scenario and compares stealthscale's compiled CapMap against
...
func testNodeAttrsError(t *testing.T, policyJSON []byte, tf *testcapture.Capture) {
	...
	// still-meaningful one: stealthscale must also refuse the policy at
	...
	for nodeName, capture := range tf.Captures {
		...
		t.Run(nodeName, func(t *testing.T) {
			...
			if diff := cmp.Diff(wantSelf, gotSelf, cmpopts.EquateEmpty()); diff != "" {
				t.Errorf(
					"%s/%s: SelfNode.CapMap mismatch (-tailscale +stealthscale):\n%s",
			...
			for _, peer := range capture.Netmap.Peers {
				...
				if diff := cmp.Diff(wantPeer, gotPeer, cmpopts.EquateEmpty()); diff != "" {
					t.Errorf(
						"%s/%s/peer=%s: Peer.CapMap mismatch (-tailscale +stealthscale):\n%s",
...
// [tailcfg.NodeCapMap] shape stealthscale renders, so both sides of the
