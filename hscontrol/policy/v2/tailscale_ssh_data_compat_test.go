...
// the 200 path compares stealthscale's compileSSHPolicy output node-by-node
// against the captured SSHRules; the non-200 path requires stealthscale to
...
// but stealthscale cannot yet represent. Each entry names the feature gap.
var sshSkipReasons = map[string]string{
	"ssh-b5":  "stealthscale has no passkey authentication; user:*@passkey wildcard unsupported",
	"ssh-d10": "stealthscale has no passkey authentication; user:*@passkey wildcard unsupported",
}

// sshRejectSkipReasons documents captures the upstream control plane
// rejects for reasons stealthscale cannot apply. Each entry names the
// feature gap.
var sshRejectSkipReasons = map[string]string{
	"ssh-b4": "stealthscale has no associated-tailnet-domains config; user:*@domain / localpart:*@domain are not domain-validated",
	"ssh-d1": "stealthscale has no associated-tailnet-domains config; user:*@domain / localpart:*@domain are not domain-validated",
	"ssh-e1": "stealthscale has no associated-tailnet-domains config; user:*@domain / localpart:*@domain are not domain-validated",
	"ssh-e2": "stealthscale has no associated-tailnet-domains config; user:*@domain / localpart:*@domain are not domain-validated",
	"ssh-malformed-user-localpart-multi-glob": "stealthscale has no associated-tailnet-domains config; user:*@domain / localpart:*@domain are not domain-validated",
}
...
// recorded error body must appear as a substring of stealthscale's
// rejection.
		...
		t.Run(tf.TestID, func(t *testing.T) {
			...
			for nodeName, capture := range tf.Captures {
				t.Run(nodeName, func(t *testing.T) {
					...
					// Compile stealthscale SSH policy for this node
					...
					// Compare stealthscale output against Tailscale expected.
					...
					if diff := cmp.Diff(wantSSH, gotSSH, opts...); diff != "" {
						t.Errorf(
							"%s/%s: SSH policy mismatch (-tailscale +stealthscale):\n%s",
							tf.TestID,
							nodeName,
							diff,
						)
					...
					// rules. This catches regressions where stealthscale
