...
// stealthscale's grants engine output against the captured packet filter rules.
//
...
// skipped with a TODO comment explaining the root cause. As stealthscale's grants
...
// Source format:    github.com/tomiwebpro/stealthscale/hscontrol/types/testcapture
...
import (
	...
	"github.com/tomiwebpro/stealthscale/hscontrol/policy/policyutil"
...
var grantSkipReasons = map[string]string{
	...
	// all passkey-authenticated users. stealthscale does not support passkey
	// authentication and has no equivalent for this wildcard pattern.
	"grant-k20": "USER_PASSKEY_WILDCARD: src=user:*@passkey not supported in stealthscale",
	"grant-k21": "USER_PASSKEY_WILDCARD: dst=user:*@passkey not supported in stealthscale",
}

// TestGrantsCompat is a data-driven test that loads all GRANT-*.json
// test files captured from Tailscale SaaS and compares stealthscale's grants
...
//
// The test converts Tailscale user email formats to stealthscale format
...
//
// 2 tests are skipped for user:*@passkey wildcard (not supported in stealthscale).
...
// assertGrantErrorContains requires that stealthscale's error contains
...
func testGrantSuccess(
	...
	for nodeName, capture := range tf.Captures {
		t.Run(nodeName, func(t *testing.T) {
			...
			// Compile stealthscale filter rules for this node
			...
			// Compare stealthscale output against Tailscale expected output.
			// The diff labels show (-tailscale +stealthscale) to make clear
			// which side produced which output.
			// EquateEmpty treats nil and empty slices as equal since
			// Tailscale's JSON null -> nil, stealthscale may return empty slice.
			opts := append(cmpOptions(), cmpopts.EquateEmpty())
			if diff := cmp.Diff(wantRules, gotRules, opts...); diff != "" {
				t.Errorf(
					"%s/%s: filter rules mismatch (-tailscale +stealthscale):\n%s",
					tf.TestID,
					nodeName,
					diff,
				)
