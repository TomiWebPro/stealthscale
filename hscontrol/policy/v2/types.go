...
// SaaS imposes no minimum (0s is accepted) so stealthscale matches.
const (
...
var (
	...
	ErrNodeAttrUnsupported          = errors.New("nodeAttrs uses a feature stealthscale does not yet support")
	ErrNodeAttrIPPoolUnsupported    = errors.New("nodeAttrs ipPool requires the IP allocator (https://github.com/tomiwebpro/stealthscale/issues/2912)")
	ErrNodeAttrTargetUnsupported    = errors.New("nodeAttrs target alias type is not supported")
)

// nodeAttrUnsupportedCaps lists caps that stealthscale parses but cannot act on
// today. Each entry maps to the tracking issue an operator can follow. The
// caps are accepted by Tailscale SaaS, but delivering them via stealthscale
...
var nodeAttrUnsupportedCaps = map[tailcfg.NodeCapability]string{
	tailcfg.NodeAttrFunnel: "https://github.com/tomiwebpro/stealthscale/issues/2527",
}
...
var (
	...
	ErrAutogroupNotSupported       = errors.New("autogroup not supported in stealthscale")
...
// contain all the IP addresses that the [Prefix] represents within StealthScale.
...
func (ag *AutoGroup) resolve(p *Policy, users types.Users, nodes views.Slice[types.NodeView]) (*netipx.IPSet, error) {
	...
	case AutoGroupNonRoot:
		...
		// This is not supported in stealthscale and requires OS-level user detection.
...
type Alias interface {
	...
	// contain all the IP addresses that the [Alias] represents within StealthScale.
...
// stealthscale-admin uses # in some field names to add metadata, so we will ignore
// those to ensure it doesnt break.
// https://github.com/GoodiesHQ/stealthscale-admin/blob/214a44a9c15c92d2b42383f131b51df10c84017c/src/lib/common/acl.svelte.ts#L38
...
func (p *Policy) validate() error {
	...
	for _, na := range p.NodeAttrs {
		...
		// compile to a no-op); stealthscale follows suit so policies
