...
// nodes:
//
//   - alice (id 1) at stealthscale.net owns alice-laptop and alice-tablet
//   - bob   (id 2) at stealthscale.net owns bob-laptop
...
func sshTestUsers() types.Users {
	return types.Users{
		{Model: gorm.Model{ID: 1}, Name: "alice", Email: "alice@stealthscale.net"},
		{Model: gorm.Model{ID: 2}, Name: "bob", Email: "bob@stealthscale.net"},
...
func TestRunSSHTests(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			policy: `{
				"tagOwners": { "tag:server": ["alice@stealthscale.net"] },
				"ssh": [{
					"action": "accept",
					"src":    ["alice@stealthscale.net"],
					"dst":    ["tag:server"],
					"users":  ["root"]
				}],
				"sshTests": [{
		...
		},
		{
			...
			wantPass:   false,
			wantErrSub: []string{"alice@stealthscale.net", "root", "expected ALLOWED"},
		},
		{
			...
			wantPass:   false,
			wantErrSub: []string{"alice@stealthscale.net", "mallory", "expected ALLOWED"},
		},
		{
			...
			policy: `{
				...
				"sshTests": [{
					"src":    "bob@stealthscale.net",
					"dst":    ["tag:server"],
					"accept": ["root"]
				}]
			}`,
			wantPass:   false,
			wantErrSub: []string{"bob@stealthscale.net", "root", "expected ALLOWED"},
		},
		{
		...
		},
		{
			...
			wantPass:   false,
			wantErrSub: []string{"alice@stealthscale.net", "root", "expected DENIED"},
		},
		{
		...
		},
		{
			...
			policy: `{
				"tagOwners": {
					"tag:server": ["alice@stealthscale.net"],
					"tag:prod":   ["alice@stealthscale.net"]
				},
				"ssh": [{
					"action": "accept",
					"src":    ["alice@stealthscale.net"],
		...
		},
		{
			...
			wantPass:   false,
			wantErrSub: []string{"alice@stealthscale.net", "alice", "expected ALLOWED"},
		},
		{
		...
		},
		{
			...
			wantErrSub: []string{"bob@stealthscale.net", "root"},
		},
		{
			...
			policy: `{
				...
				"ssh": [{
					...
					"src":    ["alice@stealthscale.net"],
					"dst":    ["tag:server"],
					"users":  ["localpart:*@stealthscale.net"]
				}],
				"sshTests": [{
					"src":    "alice@stealthscale.net",
					"dst":    ["tag:server"],
		...
		},
		{
			...
			wantPass:   false,
			wantErrSub: []string{"alice@stealthscale.net", "no accept, deny, or check"},
		},
		{
			...
			wantPass:   false,
			wantErrSub: []string{"alice@stealthscale.net", "expected ALLOWED"},
		},
		{
			...
			policy: `{
				"tagOwners": { "tag:empty": ["alice@stealthscale.net"] },
				"sshTests": [{
					"src":    "alice@stealthscale.net",
					"dst":    ["tag:empty"],
					"accept": ["root"]
		...
		},
		{
			...
			policy: `{
				"tagOwners": { "tag:prod": ["alice@stealthscale.net"] },
				"sshTests": [{
					"src":    "tag:prod",
					"dst":    ["autogroup:self"],
					"accept": ["root"]
...
func TestSetPolicyAggregatesACLAndSSHTestFailures(t *testing.T) {
	...
	assert.Contains(t, body, "bob@stealthscale.net",
...
func TestSSHPolicyTestResultsErrorsRendering(t *testing.T) {
	...
	for _, sub := range []string{
		"alice@stealthscale.net/root -> server: expected ALLOWED, got DENIED",
		"bob@stealthscale.net/root -> alice-laptop: expected DENIED, got ALLOWED",
		"alice@stealthscale.net/ubuntu -> server: expected ALLOWED via check, got ALLOWED via accept",
	} {
