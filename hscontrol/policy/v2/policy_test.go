...
import (
	...
	"github.com/tomiwebpro/stealthscale/hscontrol/policy/matcher"
...
func TestPolicyManager(t *testing.T) {
	users := types.Users{
		{Model: gorm.Model{ID: 1}, Name: "testuser", Email: "testuser@stealthscale.net"},
		{Model: gorm.Model{ID: 2}, Name: "otheruser", Email: "otheruser@stealthscale.net"},
	}
...
func TestInvalidateAutogroupSelfCache(t *testing.T) {
	users := types.Users{
		{Model: gorm.Model{ID: 1}, Name: "user1", Email: "user1@stealthscale.net"},
		{Model: gorm.Model{ID: 2}, Name: "user2", Email: "user2@stealthscale.net"},
		{Model: gorm.Model{ID: 3}, Name: "user3", Email: "user3@stealthscale.net"},
	}
...
func TestAutogroupSelfReducedVsUnreducedRules(t *testing.T) {
	user1 := types.User{Model: gorm.Model{ID: 1}, Name: "user1", Email: "user1@stealthscale.net"}
	user2 := types.User{Model: gorm.Model{ID: 2}, Name: "user2", Email: "user2@stealthscale.net"}
...
// https://github.com/tomiwebpro/stealthscale/issues/2389
func TestTagPropagationToPeerMap(t *testing.T) {
	...
	policy := `{
		"tagOwners": {
			"tag:web": ["user1@stealthscale.net"],
			"tag:internal": ["user1@stealthscale.net"]
		},
		"acls": [
			{
				"action": "accept",
				"src": ["user2@stealthscale.net"],
				"dst": ["user2@stealthscale.net:*"]
			},
			{
				"action": "accept",
				"src": ["user2@stealthscale.net"],
...
// juanfont/stealthscale#3212. An ACL that grants access only via
...
func TestBuildPeerMap_AutogroupInternetMakesExitNodeVisible(t *testing.T) {
	...
	policy := `{
		"acls": [
			{"action": "accept", "src": ["alice@stealthscale.net"], "dst": ["autogroup:internet:*"]}
		]
	}`
...
// Empty users → syntax-only check, used by `stealthscale policy check`.
...
// https://github.com/tomiwebpro/stealthscale/issues/3256.
//
...
func TestPeerRelayGrantMakesRelayVisible(t *testing.T) {
	users := types.Users{
		{Model: gorm.Model{ID: 1}, Name: "alice", Email: "alice@stealthscale.net"},
		{Model: gorm.Model{ID: 2}, Name: "tagowner", Email: "tagowner@stealthscale.net"},
	}
	...
	tests := []struct {
		...
		},
		{
			...
			policy: `{
				"tagOwners": {
					"tag:us-east-vpc":    ["tagowner@stealthscale.net"],
					"tag:us-east-relays": ["tagowner@stealthscale.net"]
				},
				"grants": [
					{
						"src": ["tag:us-east-vpc"],
		...
		},
		{
			...
			policy: `{
				"tagOwners": {
					"tag:client": ["tagowner@stealthscale.net"]
				},
				"grants": [
					{
						"src": ["tag:client"],
