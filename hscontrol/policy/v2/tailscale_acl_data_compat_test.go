...
// compares stealthscale's ACL engine output against the expected packet filter
...
// and compares stealthscale's ACL engine output against the expected behavior.
//
...
// assertACLErrorContains requires that stealthscale's error contains the
...
func testACLSuccess(
	...
	// Convert Tailscale SaaS user emails to stealthscale @example.com format.
	policyJSON := convertPolicyUserEmails(tf.Input.FullPolicy)
