...
// captured from a Tailscale-hosted control plane, and compares stealthscale's route-aware
...
// files and compares stealthscale's route-aware ACL engine output against the
// expected behavior.
func TestRoutesCompat(t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(tf.TestID, func(t *testing.T) {
			...
			// Convert Tailscale SaaS user emails to stealthscale format
...
// TestRoutesCompatAutoApproval validates that stealthscale's auto-approval
...
// TestRoutesCompatReduceRoutes validates that stealthscale's CanAccessRoute
...
// TestRoutesCompatNoPeersBeyondCaptures verifies that stealthscale does not
...
// TestRoutesCompatPeerAllowedIPs validates that stealthscale computes the same
...
// for each peer against what stealthscale's ReduceRoutes (the core of
...
func TestRoutesCompatPeerAllowedIPs(t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(tf.TestID, func(t *testing.T) {
			...
			// SaaS rejected this policy — verify stealthscale also rejects it.
			if tf.Error {
			...
			for viewerName, capture := range tf.Captures {
				...
				t.Run(viewerName, func(t *testing.T) {
					...
					for _, nmPeer := range capture.Netmap.Peers {
						...
						// Compute what stealthscale would put in AllowedIPs.
						//
...
// assertRoutesErrorContains requires that stealthscale's error contains
