...
func TestRunTests(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantPass:    false,
			wantErrSub:  []string{"bob@stealthscale.net", "tag:server:22", "expected ALLOWED"},
		...
		},
		{
			...
			wantPass:    false,
			wantErrSub:  []string{"alice@stealthscale.net", "tag:server:22", "expected DENIED"},
			wantNoErrIs: errPolicyTestsFailed,
		},
		{
			...
			policy: `{
				...
				"tests": [{
					"src": "ghost@stealthscale.net",
					"accept": ["alice-laptop:22"]
				}]
			}`,
			wantPass:    false,
			wantErrSub:  []string{"ghost@stealthscale.net", "failed to resolve source"},
			wantNoErrIs: errPolicyTestsFailed,
		},
		...
		},
		{
			...
			wantPass:    false,
			wantErrSub:  []string{"bob@stealthscale.net", "expected ALLOWED"},
			wantNoErrIs: errPolicyTestsFailed,
		},
		{
...
func TestNewPolicyManagerSkipsTests(t *testing.T) {
	...
	// Tests reference "ghost@stealthscale.net" which doesn't exist. Boot
	// must not error.
	stale := `{
		"acls": [{
			"action": "accept",
...
func TestPolicyTestResultsErrorsRendering(t *testing.T) {
	...
	for _, sub := range []string{
		"alice@stealthscale.net -> tag:server:22: expected ALLOWED, got DENIED",
		"bob@stealthscale.net -> tag:server:443 (tcp): expected DENIED, got ALLOWED",
	} {
