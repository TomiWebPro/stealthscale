...
//     captured Message is the byte-exact body the user saw, and stealthscale
...
//     block passed); stealthscale's RunTests must also pass.
//
...
// these are real Tailscale ↔ stealthscale divergences uncovered by the
...
// knownPolicyTesterDivergences lists scenarios where stealthscale's evaluator
...
func TestPolicyTesterCompat(t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(c.TestID, func(t *testing.T) {
			...
			// either failure mode produces the same 400. StealthScale
			...
			if c.Input.APIResponseCode == 200 {
				require.NoError(t, parseErr, "tailscale accepted this policy; stealthscale must parse it")

				_, setErr := pm.SetPolicy(policyJSON)
				require.NoError(t, setErr, "tailscale accepted this policy; stealthscale tests should pass")
