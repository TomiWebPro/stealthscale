...
`,
			want: &Policy{
				Groups: Groups{
					Group("group:example"): []Username{Username("derp@stealthscale.net")},
				},
			},
		},
		{
			...
			input: `
{
	"groups": {
		"group:example": [
			"testuser@stealthscale.net",
		],
		"group:other": [
			"otheruser@stealthscale.net",
		],
		"group:noat": [
			"noat@",
		],
	...
	"tagOwners": {
		"tag:user": ["testuser@stealthscale.net"],
		"tag:group": ["group:other"],
		"tag:userandgroup": ["testuser@stealthscale.net", "group:other"],
	},
	...
	"acls": [
		...
		// Users
		{
			"action": "accept",
			"proto": "tcp",
			"src": ["testuser@stealthscale.net"],
			"dst": ["otheruser@stealthscale.net:80"],
		},
		// Groups
		{
			"action": "accept",
...
`,
			want: &Policy{
				Groups: Groups{
					Group("group:example"): []Username{Username("testuser@stealthscale.net")},
					Group("group:other"):   []Username{Username("otheruser@stealthscale.net")},
					Group("group:noat"):    []Username{Username("noat@")},
				},
				TagOwners: TagOwners{
					Tag("tag:user"):         Owners{up("testuser@stealthscale.net")},
					Tag("tag:group"):        Owners{gp("group:other")},
					Tag("tag:userandgroup"): Owners{up("testuser@stealthscale.net"), gp("group:other")},
				},
				Hosts: Hosts{
					"host-1":   Prefix(mp("100.100.100.100/32")),
				...
				ACLs: []ACL{
					...
					},
					{
						...
						Protocol: "tcp",
						Sources: Aliases{
							new(Username("testuser@stealthscale.net")),
						},
						Destinations: []AliasWithPorts{
							{
								Alias: new(Username("otheruser@stealthscale.net")),
								Ports: []tailcfg.PortRange{{First: 80, Last: 80}},
							},
						},
					},
...
func TestSSHRuleSaaSValidation(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			// `invalid dst "srv"`. stealthscale validates the same
