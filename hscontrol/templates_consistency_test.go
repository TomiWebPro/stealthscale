	...
	testCases := []struct {
	...
	}{
		{
			...
			html: templates.AuthSuccess(templates.AuthSuccessResult{
				Title:   "StealthScale - Node Registered",
		...
		},
		{
			...
			html: templates.AuthWeb(
				...
				"Run the command below in the stealthscale server to add this machine to your network:",
				"stealthscale auth register --auth-id test-key-123 --user USERNAME",
			).Render(),
		},
		{
			...
			html: templates.AuthWeb(
				...
				"stealthscale auth approve --auth-id test-key-123",
			).Render(),
		},
		{
			name: "Windows Config",
