...
// stay stable across runs; refresh with STEALTHSCALE_UPDATE_GOLDEN=1 after an
...
func newHumaTestHandler(app *StealthScale) http.Handler {
...
func assertParityIsolated(
	t *testing.T,
	seed func(t *testing.T, app *StealthScale),
	method, path string,
	body []byte,
) httpResult {
	t.Helper()
...
func assertAgainstGolden(t *testing.T, method, path string, hum httpResult) {
	...
	if os.Getenv("STEALTHSCALE_UPDATE_GOLDEN") != "" {
	...
	require.NoErrorf(t, err,
		"missing golden %s for %s %s — run with STEALTHSCALE_UPDATE_GOLDEN=1 to generate",
		gpath, method, path)
...
// writeGolden persists the response under STEALTHSCALE_UPDATE_GOLDEN. Success
