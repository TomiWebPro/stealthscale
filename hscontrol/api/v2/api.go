// Package apiv2 is StealthScale's v2 HTTP API, served at /api/v2.
//
// Where the v1 API (hscontrol/api/v1) is the stealthscale-native admin surface, v2
...
// tailscale.com/client/tailscale/v2) can drive StealthScale unchanged. Ported
// operations carry the "Tailscale compat" tag; a stealthscale-native v2 operation
// may use stealthscale's own conventions instead. See README.md for the porting
// guide.
//
...
func Config() huma.Config {
	config := huma.DefaultConfig("StealthScale API", "v2")
	config.Info.Description = "StealthScale v2 API. Some endpoints are ported from / compatible with the Tailscale API (tagged \"Tailscale compat\")."
...
// requireDefaultTailnet rejects any tailnet other than "-". StealthScale is
...
const scopeMetaKey = "stealthscale.scope"
