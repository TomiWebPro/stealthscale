# API v2: StealthScale's v2 API

This is StealthScale's v2 HTTP API, served at `/api/v2`. Some of its endpoints are
**ported from Tailscale's API**, reusing Tailscale's wire shapes, so the
Tailscale ecosystem that cannot talk to StealthScale today works: the
...
at a time, only as we need them; a stealthscale-native v2 endpoint may use
stealthscale's own conventions. The stealthscale-native admin API stays at `/api/v1`
...
- The `{tailnet}` path segment must be `-` (the single StealthScale tailnet);
...
- Errors use **Tailscale's** body (`{"message","data","status"}`), installed as
  ...
  stealthscale-native v2 operation would keep Huma's RFC 9457 problem+json.
...
operator is OAuth-only. Supporting OAuth lets all of them drive StealthScale.
...
Worked example: the keys resource (`keys.go`) = Tailscale auth keys = StealthScale
pre-auth keys.
...
1. **Read the Tailscale spec.** Find the operation in the [Tailscale API
   ...
   request/response schema, and which variant(s) StealthScale supports (auth keys
   only, for keys).
...
3. **Map to StealthScale.** Write the field ↔ field ↔ `state` call mapping. Record
   gaps and the decision for each (e.g. Tailscale `preauthorized` has no
   StealthScale equivalent: accepted, ignored, echoed back). _Acceptance: every
...
7. **Update the CLI** only if the v2 operation fully replaces a v1 one. Tailscale
   ...
   equivalent of v1 `preauthkeys expire`. `stealthscale preauthkeys` still stays on
