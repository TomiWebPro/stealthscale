...
// StealthScale models none of type/role/status and has a single tailnet, so these
...
// User is the Tailscale user response. Identity fields map from the StealthScale
...
type (
	...
	listUsersInput struct {
		Tailnet string `doc:"Tailnet; must be \"-\" (the single StealthScale tailnet)."   path:"tailnet"`
		Type    string `doc:"Filter by user type; StealthScale users are all \"member\"." query:"type"`
		Role    string `doc:"Filter by user role; StealthScale users are all \"member\"." query:"role"`
	}
...
func registerUsers(api huma.API, b Backend) {
	...
	huma.Register(api, requireScope(huma.Operation{
		...
		// StealthScale has only "member" users. A filter for any other type/role
...
// matchesMember reports whether an optional type/role filter selects StealthScale's
// only user kind. An empty value means "no filter".
func matchesMember(filter string) bool {
	return filter == "" || filter == userTypeMember
}

// userFromView maps a StealthScale user onto the Tailscale User through the
