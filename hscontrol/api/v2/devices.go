...
import (
	...
	"github.com/tomiwebpro/stealthscale/hscontrol/scope"
...
// Device is the Tailscale device response. StealthScale nodes map onto it; fields
// StealthScale does not track are emitted as their zero value. The route slices
...
type (
	...
	// rather than failing to decode (StealthScale cannot untag a node).
...
func registerDevices(api huma.API, b Backend) {
	...
	huma.Register(api, requireScope(huma.Operation{
		...
		// StealthScale nodes are authorized the moment they register; there is no
		...
		if !in.Body.Authorized {
			return nil, huma.Error400BadRequest(
				"StealthScale does not support de-authorizing a device; delete or expire it instead",
			)
		}
	...
	huma.Register(api, requireScope(huma.Operation{
		...
		// StealthScale cannot make a node untagged (tags-as-identity is one-way), so
	...
	huma.Register(api, requireScope(huma.Operation{
		...
		// Re-enabling has no target expiry in the Tailscale request and StealthScale
...
// deviceFromView maps a StealthScale node onto the Tailscale Device, reading
...
func deviceFromView(view types.NodeView, allFields bool) Device {
	...
	d := Device{
		...
		ID:                id,
		NodeID:            id,
		Authorized:        true, // StealthScale has no post-registration de-auth state.
