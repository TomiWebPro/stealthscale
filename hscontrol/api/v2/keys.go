...
const (
	// keyTypeAuth is a machine auth key (StealthScale pre-auth key); the default.
	keyTypeAuth = "auth"
...
// KeyCapabilities maps a resource to the actions a key permits. StealthScale
...
type (
	...
	listKeysInput struct {
		...
		All     bool   `doc:"Accepted for compatibility; StealthScale returns all keys." query:"all"`
	}
...
// match keyFromStored: StealthScale always authorizes pre-auth-key nodes, so the
...
// never returned here. preauthorized is reported true: StealthScale has no separate
