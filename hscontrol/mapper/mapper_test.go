...
func TestDNSConfigMapResponse(t *testing.T) {
	tests := []struct {
		...
		},
		{
			magicDNS: false,
			want: &tailcfg.DNSConfig{
				Domains: []string{"foobar.stealthscale.net"},
				Proxied: false,
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(fmt.Sprintf("with-magicdns-%v", tt.magicDNS), func(t *testing.T) {
			...
			baseDomain := "foobar.stealthscale.net"
...
func TestBuildFromChangeVisibilityMatchesFullMap(t *testing.T) {
	...
	// Note deny_all: an empty ACL set compiles to zero matchers, which stealthscale
