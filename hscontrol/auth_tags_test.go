...
func createTestAppWithNodeExpiry(t *testing.T, nodeExpiry time.Duration) *StealthScale {
	t.Helper()
...
// This is the core fix for https://github.com/tomiwebpro/stealthscale/issues/1711
...
// https://github.com/tomiwebpro/stealthscale/issues/3371 through the real
...
func TestIssue3371_TaggedNodeLogoutReloginSingleUseKey(t *testing.T) {
	...
	// `stealthscale preauthkeys create --tags tag:tag1` (single-use).
	pak, err := app.state.CreatePreAuthKey(user.TypedID(), false, false, nil, tags)
	require.NoError(t, err)
	...
	regReq := tailcfg.RegisterRequest{
		...
		NodeKey:  nodeKey.Public(),
		Hostinfo: &tailcfg.Hostinfo{Hostname: "stealthscale-debug"},
	}
...
func TestIssue3371_TaggedNodeLogoutReloginReusableKey(t *testing.T) {
	...
	// `stealthscale preauthkeys create --reusable --tags tag:tag1`.
	pak, err := app.state.CreatePreAuthKey(user.TypedID(), true, false, nil, tags)
	require.NoError(t, err)
...
// set by an admin (`stealthscale nodes expire`); TestTaggedNodeCanHaveKeyExpiry
...
func TestIssue3371_TaggedNodeFutureExpirySurvivesRelogin(t *testing.T) {
	...
	// Admin sets a deliberate future expiry (`stealthscale nodes expire`).
