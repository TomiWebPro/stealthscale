...
var (
	...
	errServerURLSuffix           = errors.New("server_url cannot be part of base_domain in a way that could make the DERP and stealthscale server unreachable")
	errServerURLSame             = errors.New("server_url cannot use the same domain as base_domain in a way that could make the DERP and stealthscale server unreachable")
...
// Config contains the initial StealthScale configuration.
type Config struct {
	ServerURL           string
	Addr                string
	MetricsAddr         string
	...
	// DNSConfig is the stealthscale representation of the DNS configuration.
...
type XRayConfig struct {
	...
	// ListenAddr is the address the VLESS listeners bind to.
	ListenAddr string

	// BaseListenPort is the first port in the per-node allocation range.
	BaseListenPort int
...
// auto-update. When Enabled is true, stealthscale emits the
...
type CLIConfig struct {
	Address  string
	APIKey   string `json:"-"` // never serialise the stealthscale admin API key
	Timeout  time.Duration
	Insecure bool
}
...
// Tuning contains advanced performance tuning parameters for StealthScale.
...
type Tuning struct {
	...
	//
	// By batching writes, StealthScale can process N operations but only rebuild once,
...
// LoadConfig prepares and loads the StealthScale configuration into Viper.
...
func validateServerConfig() error {
	...
	if viper.GetString("noise.private_key_path") == "" {
		errorText += "Fatal config error: stealthscale now requires a new `noise.private_key_path` field in the config file for the Tailscale v2 protocol\n"
	}

	if viper.GetBool("xray.enabled") {
		port := viper.GetInt("xray.listen_port")
		maxPort := viper.GetInt("xray.max_listen_port")
		if port <= 0 || maxPort > 65535 || port > maxPort {
	...
	if (viper.GetString("tls_letsencrypt_hostname") != "") &&
		...
		// this is only a warning because there could be something sitting in front of stealthscale that redirects the traffic (e.g. an iptables rule)
		log.Warn().
			Msg("Warning: when using tls_letsencrypt_hostname with TLS-ALPN-01 as challenge type, stealthscale must be reachable on port 443, i.e. listen_addr should probably end in :443")
	}
...
// of StealthScale to connect to a StealthScale server.
...
// LoadServerConfig returns the full StealthScale configuration to
// host a StealthScale server. This is called as part of `stealthscale serve`.
func LoadServerConfig() (*Config, error) {
	...
	if v4NonStandard || v6NonStandard {
		warnBanner([]string{
			"You have overridden the default StealthScale IP prefixes",
	...
	// causing the stealthscale server and DERP to be unreachable.
	...
	return &Config{
		...
		XRay: XRayConfig{
			Enabled:        viper.GetBool("xray.enabled"),
