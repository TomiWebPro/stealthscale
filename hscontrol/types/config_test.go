	...
	tests := []struct {
	...
	}{
		{
			...
			configEnv: map[string]string{
				...
				"STEALTHSCALE_DATABASE_SQLITE_WRITE_AHEAD_LOG": "false",
		...
		},
		{
			...
			configEnv: map[string]string{
				...
				"STEALTHSCALE_DNS_BASE_DOMAIN":        "example.com",
				"STEALTHSCALE_DNS_OVERRIDE_LOCAL_DNS": "false",
				"STEALTHSCALE_DNS_NAMESERVERS_GLOBAL": `1.1.1.1 8.8.8.8`,
				"STEALTHSCALE_DNS_SEARCH_DOMAINS":     "test.com bar.com",

				// TODO(kradalby): Figure out how to pass these as env vars
				// "STEALTHSCALE_DNS_NAMESERVERS_SPLIT":  `{foo.bar.com: ["1.1.1.1"]}`,
				// "STEALTHSCALE_DNS_EXTRA_RECORDS":      `[{ name: "prometheus.myvpn.example.com", type: "A", value: "100.64.0.4" }]`,
			},
			setup: func(t *testing.T) (any, error) { //nolint:thelper // inline test closure
				t.Logf("all settings: %#v", viper.AllSettings())
...
// OK
// server_url: stealthscale.com, base: clients.stealthscale.com
// server_url: stealthscale.com, base: stealthscale.net
//
// NOT OK
// server_url: server.stealthscale.com, base: stealthscale.com.
func TestSafeServerURL(t *testing.T) {
	tests := []struct {
		...
		{
			serverURL:  "https://stealthscale.com",
			baseDomain: "stealthscale.com",
			wantErr:    errServerURLSame.Error(),
		},
		{
			serverURL:  "https://stealthscale.com",
			baseDomain: "clients.stealthscale.com",
		},
		{
			serverURL:  "https://stealthscale.com",
			baseDomain: "clients.subdomain.stealthscale.com",
		},
		{
			serverURL:  "https://stealthscale.kristoffer.com",
			baseDomain: "mybase",
		},
		{
			serverURL:  "https://server.stealthscale.com",
			baseDomain: "stealthscale.com",
			wantErr:    errServerURLSuffix.Error(),
		},
		{
			serverURL:  "https://server.subdomain.stealthscale.com",
			baseDomain: "stealthscale.com",
			wantErr:    errServerURLSuffix.Error(),
		},
		{
...
func TestSafeServerURLWithPort(t *testing.T) {
	tests := []struct {
		...
		{
			serverURL:  "https://server.stealthscale.com:443",
			baseDomain: "stealthscale.com",
			wantErr:    errServerURLSuffix.Error(),
		},
		{
			serverURL:  "https://server.subdomain.stealthscale.com:8080",
			baseDomain: "stealthscale.com",
			wantErr:    errServerURLSuffix.Error(),
		},
		{
			serverURL:  "https://stealthscale.com:443",
			baseDomain: "stealthscale.com",
			wantErr:    errServerURLSame.Error(),
		},
		{
...
// Postgres password, the OIDC client secret, or the stealthscale admin
