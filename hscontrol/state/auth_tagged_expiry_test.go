...
func TestTaggedReauthKeepsNilExpiry(t *testing.T) {
	dbPath := t.TempDir() + "/stealthscale.db"
	cfg := persistTestConfig(dbPath)

	database, err := db.NewStealthScaleDatabase(cfg)
...
//
// https://github.com/tomiwebpro/stealthscale/issues/3312
func TestTaggedReauthWithReusedUserPAK(t *testing.T) {
	...
	// One-shot, user-owned PAK: `stealthscale preauthkeys create -u 1`.
	...
	// `stealthscale nodes tag -t tag:foo`: convert to a tagged node. This clears
...
// via group:ci) owns BOTH tags. Today stealthscale rejects the whole set as
...
// interactive/OIDC arm of https://github.com/tomiwebpro/stealthscale/issues/3371
...
// upgrade path: a tagged node broken by an OLDER stealthscale carries a past
...
// `stealthscale nodes tag`. This is the unit-level guard for the integration
...
// (User != nil, the `stealthscale preauthkeys create -u <user> --tags` shape)
...
// node): a deliberate FUTURE expiry set by an admin via `stealthscale nodes expire`
...
func TestTaggedPAKReauthRetagPreservesFutureAdminExpiry(t *testing.T) {
	...
	// (`stealthscale nodes expire`), which SetNodeExpiry permits.
...
func newRetagTestState(t *testing.T) *State {
	...
	cfg := persistTestConfig(t.TempDir() + "/stealthscale.db")
	s, err := NewState(cfg)
	require.NoError(t, err)
	t.Cleanup(func() { _ = s.Close() })
