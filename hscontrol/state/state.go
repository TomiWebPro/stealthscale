// Package state provides core state management for StealthScale,
...
import (
	...
	hsdb "github.com/tomiwebpro/stealthscale/hscontrol/db"
	...
	"github.com/tomiwebpro/stealthscale/hscontrol/util/zlog/zf"
	"gorm.io/gorm"
...
// State manages StealthScale's core state, coordinating between database, policy management,
...
type State struct {
	// cfg holds the current StealthScale configuration
	cfg *types.Config
...
func NewState(cfg *types.Config) (*State, error) {
	...
	db, err := hsdb.NewStealthScaleDatabase(cfg)
	if err != nil {
...
func (s *State) applyAuthNodeUpdate(params authNodeUpdateParams) (types.NodeView, error) {
	...
	updatedNodeView, ok := s.nodeStore.UpdateNode(params.ExistingNode.ID(), func(node *types.Node) {
		...
		case isTagged && node.IsExpired():
			...
			// stealthscale's logout stamp (#3371). Tagged nodes never expire, so
...
func (s *State) findExistingNodeForPAK(
	...
	if pak.User != nil {
		...
		// ownership. See https://github.com/tomiwebpro/stealthscale/issues/3312.
...
func (s *State) HandleNodeFromPreAuthKey(
	...
	// older stealthscale (#3371). Forcing it down the re-validation path burns its
	...
	if existsSameUser && existingNodeSameUser.Valid() {
		...
		updatedNodeView, ok := s.nodeStore.UpdateNode(existingNodeSameUser.ID(), func(node *types.Node) {
			...
			if pak.IsTagged() && (!node.IsTagged() || keyChanged) {
				...
				// set via `stealthscale nodes expire` - that is a node property, not
			...
			if !node.IsTagged() {
				...
				// stale PAST expiry left by a logout (older stealthscale) so
				// re-auth is not permanently blocked. A deliberate future
				// expiry (stealthscale nodes expire) has IsExpired() == false and
				// is left untouched.
				node.Expiry = nil
			}
		})
...
//
// Limitation: stealthscale stores bare []netip.AddrPort with no per-endpoint
