...
const (
	...
	DerpVerifyScheme = "stealthscale-derp-verify"
)
...
var debugUseDERPIP = envknob.Bool("STEALTHSCALE_DEBUG_DERP_USE_IP")
...
func (d *DERPServer) GenerateRegion() (tailcfg.DERPRegion, error) {
	...
	if debugUseDERPIP {
		...
		if err != nil {
			...
			log.Info().Caller().Msgf("STEALTHSCALE_DEBUG_DERP_USE_IP: resolved %s to %s", host, ipStr)
			host = ipStr
		}
	}
...
func (d *DERPServer) DERPHandler(
	...
	if upgrade != "websocket" && upgrade != "derp" {
		if upgrade != "" {
			log.Warn().
				Caller().
				Msg("No Upgrade header in DERP server request. If stealthscale is behind a reverse proxy, make sure it is configured to pass WebSockets through.")
		}
...
// They have a cache, but not clear if that is really necessary at StealthScale, uh, scale.
