...
func (h *StealthScale) handleRegister(
	ctx context.Context,
	...
	if req.Auth == nil {
		...
		if node, ok := h.state.GetNodeByNodeKey(req.NodeKey); ok {
			...
			// [StealthScale.handleLogout] enforces the same check on its own path.
...
func (h *StealthScale) handleLogout(
	node types.NodeView,
	req tailcfg.RegisterRequest,
	machineKey key.MachinePublic,
) (*tailcfg.RegisterResponse, error) {
	...
	// Zero expiry is handled in [StealthScale.handleRegister] before calling this function.
	...
	// admin path `stealthscale nodes expire` remains free to set a deliberate
...
func nodeToRegisterResponse(node types.NodeView) *tailcfg.RegisterResponse {
	resp := &tailcfg.RegisterResponse{
		...
		// StealthScale does not implement the concept of machine authorization
...
func (h *StealthScale) waitForFollowup(
	ctx context.Context,
	req tailcfg.RegisterRequest,
	machineKey key.MachinePublic,
) (*tailcfg.RegisterResponse, error) {
	...
	if reg, ok := h.state.GetAuthCacheEntry(followupReg); ok {
		...
		if verdict.Accept() {
			...
			// match on the normal path. [StealthScale.handleRegister] and
			// [StealthScale.handleLogout] apply the same check.
			err := machineKeyMismatch(verdict.Node, machineKey)
			if err != nil {
				return nil, err
			}

			h.ensureXRayListenerForNode(verdict.Node.ID())
...
func (h *StealthScale) reqToNewRegisterResponse(
	req tailcfg.RegisterRequest,
	machineKey key.MachinePublic,
) (*tailcfg.RegisterResponse, error) {
...
func (h *StealthScale) handleRegisterWithAuthKey(
	req tailcfg.RegisterRequest,
	machineKey key.MachinePublic,
) (*tailcfg.RegisterResponse, error) {
...
func (h *StealthScale) handleRegisterInteractive(
	req tailcfg.RegisterRequest,
	machineKey key.MachinePublic,
) (*tailcfg.RegisterResponse, error) {
